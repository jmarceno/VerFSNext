...
impl<'a> ReplyDirectoryPlus<'a> {
    ...
    /// Add an entry to the directory plus reply buffer. Returns true if the
    /// buffer is full.
    ...
    pub fn add<T: AsRef<OsStr>>(
        ...
        let entlen = direntplus.size_with_name();
        let entsize = crate::util::round_up(entlen, mem::size_of::<u64>());
...
/// Get the underlying raw part of a `FuseDirEntPlus`, represented in `&[u8]`.
///
/// # Safety
/// Behavior is undefined if any of the following conditions are violated:
/// - `from` must be a valid reference to `FuseDirEntPlus`.
/// - The `namelen` field in `from.dirent` must represent a valid length of the
///   nearly following name bytes in a continuous allocation.
#[cfg(feature = "abi-7-21")]
unsafe fn fuse_dir_ent_plus_in_raw(from: &FuseDirEntPlus) -> &[u8] {
    let base: *const u8 = <*const FuseDirEntPlus>::cast(from);
    unsafe {
        let bytes = slice::from_raw_parts(base, mem::size_of::<FuseDirEntPlus>());
        bytes
    }
}
