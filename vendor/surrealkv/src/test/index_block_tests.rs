    ...
    for (key, expected) in test_cases.iter() {
        ...
        match expected {
            Some(expected_sep_key) => {
                ...
                assert_eq!(
                    &handle.separator_key, expected_sep_key,
...
fn test_find_block_handle_returns_correct_partition_index() {
    ...
    for (expected_idx, sep_key) in sep_keys.iter().enumerate() {
        ...
        assert_eq!(
            idx, expected_idx,
            "Returned index should match partition index"
        );
        assert_eq!(
...
fn test_partition_lookup_empty_partition_returns_none() {
    ...
    assert!(
        result.is_none(),
        "Should return None for key beyond all partitions"
    );
}
...
fn test_partition_lookup_single_partition() {
    ...
    let index = Index {
        id: 0,
        opts,
        blocks: vec![BlockHandleWithKey::new(
            sep.clone(),
...
fn test_partitioned_index_seek_correctness() {
    ...
    let test_keys = vec![
        "0015", "0035", "0054", "0055", "0056", "0057", "0058", "0075", "0076", "0095",
    ];
    ...
    for key in &non_existing_keys {
        ...
        if let Some((found_key, _)) = result {
            ...
            assert!(
                test_keys.contains(&found_str),
                "Found key {found_str} should be in test_keys"
            );
        }
    }
    ...
    backward_keys.push(
        std::str::from_utf8(iter.key().user_key())
            .unwrap()
            .to_string(),
    );
    ...
    for (idx, block) in partitioned_index.blocks.iter().enumerate() {
        ...
        if idx > 0 {
            ...
            if result.is_some() {
                ...
                assert!(
                    found_key.user_key <= sep_key.user_key,
                    "Found key should be <= separator"
                );
            }
        }
    }
...
fn test_partitioned_index_reseek() {
    ...
    assert!(
        iter.valid(),
        "Iterator should be valid after second seek_to_last"
    );
    assert_eq!(
        std::str::from_utf8(iter.key().user_key()).unwrap(),
        "0095",
...
fn test_partitioned_index_varying_partition_sizes() {
    ...
    for partition_size in 1..=est_max_index_size + 1 {
        ...
        for i in 0..20 {
            ...
            assert!(
                result.is_some(),
                "Should find key {key} at partition_size {partition_size}"
            );
            if let Some((found_key, found_value)) = result {
                assert_eq!(
                    std::str::from_utf8(&found_key.user_key).unwrap(),
        ...
        assert_eq!(
            count, 20,
            "Should iterate all 20 keys at partition_size {partition_size}"
        );
    }
}
