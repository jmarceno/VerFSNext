...
fn test_batch_large_key_value() {
    ...
    let mut batch = Batch::new(1);
    batch
        .set(large_key.clone(), large_value.clone(), 1)
        .unwrap();
...
fn test_batch_unicode_keys_and_values() {
    let mut batch = Batch::new(1);
    batch
        .set("🔑".as_bytes().to_vec(), "🗝️".as_bytes().to_vec(), 1)
        .unwrap();
    batch
        .set(
...
fn test_batch_large_number_of_records() {
    ...
    for i in 0..NUM_RECORDS {
        ...
        if i % 2 == 0 {
            batch
                .set(key.as_bytes().to_vec(), value.as_bytes().to_vec(), i as u64)
                .unwrap();
        } else {
...
fn test_batch_version() {
    ...
    batch_with_data
        .set(b"key".to_vec(), b"value".to_vec(), 1)
        .unwrap();
...
fn test_add_record_consistent_encoding() {
    ...
    batch_none
        .add_record(InternalKeyKind::Delete, b"test_key".to_vec(), None, 100)
        .unwrap();
    ...
    assert_eq!(
        entries_merge[0].value.as_ref().unwrap().as_slice(),
        b"merge_data"
    );
    ...
    assert_eq!(
        entries_set[0].value.as_ref().unwrap().as_slice(),
        b"set_data"
    );
...
fn test_batch_encode_decode() {
    ...
    batch
        ...
        .unwrap();
    batch
        .add_record(InternalKeyKind::Delete, b"key2".to_vec(), None, 2)
        .unwrap();
    batch
        .add_record(
            InternalKeyKind::Merge,
    ...
    batch
        .add_record(
            InternalKeyKind::Set,
            b"empty_value_key".to_vec(),
            Some(b"".to_vec()),
            8,
        )
        .unwrap();
    batch
        .add_record(InternalKeyKind::Set, b"".to_vec(), Some(b"".to_vec()), 9)
        .unwrap();
    ...
    assert_eq!(entries[2].key.as_slice(), b"key3");
    assert_eq!(
        entries[2].value.as_ref().unwrap().as_slice(),
        b"merge_value"
    );
    assert_eq!(entries[2].timestamp, 3);
    ...
    assert_eq!(
        entries[3].value.as_ref().unwrap().as_slice(),
        b"large_value"
    );
    ...
    assert_eq!(
        entries[6].value.as_ref().unwrap().as_slice(),
        b"empty_key_value"
    );
    ...
    assert_eq!(
        entries[9].value.as_ref().unwrap().as_slice(),
        "🗝️".as_bytes()
    );
    ...
    assert_eq!(
        entries[10].value.as_ref().unwrap().as_slice(),
        "世界".as_bytes()
    );
    ...
    assert_eq!(
        encoded, re_encoded,
        "Re-encoding should produce identical result"
    );
}
