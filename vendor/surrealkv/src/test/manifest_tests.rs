...
fn test_level_manifest_persistence() {
    ...
    assert_eq!(
        loaded_manifest.snapshots.len(),
        2,
        "Snapshots not persisted correctly"
    );
    assert_eq!(
    ...
    assert_eq!(
        new_manifest.snapshots.len(),
        2,
        "Snapshots not loaded correctly"
    );
    assert_eq!(
        new_manifest.snapshots[0].seq_num, 10,
        "First snapshot not loaded correctly"
    );
    assert_eq!(
        new_manifest.snapshots[1].seq_num, 20,
        "Second snapshot not loaded correctly"
    );
    assert_eq!(
        new_manifest.path,
        opts.manifest_file_path(0),
    ...
    assert!(
        props1.data_size > 0,
        "Table 1 data size should be greater than 0"
    );
    assert!(props1.created_at > 0, "Table 1 created_at should be set");
    assert_eq!(
        props1.item_count, 100,
        "Table 1 item count should match entries"
    );
    assert_eq!(
        props1.key_count, 100,
        "Table 1 key count should match entries"
    );
    assert!(
        props1.block_count > 0,
        "Table 1 should have at least one block"
    );
    assert_eq!(
        props1.seqnos,
        (1, 100),
        "Table 1 sequence numbers should be (1, 100)"
    );
    ...
    assert!(
        props2.data_size > 0,
        "Table 2 data size should be greater than 0"
    );
    assert!(props2.created_at > 0, "Table 2 created_at should be set");
    assert_eq!(
        props2.item_count, 200,
        "Table 2 item count should match entries"
    );
    assert_eq!(
        props2.key_count, 200,
        "Table 2 key count should match entries"
    );
    assert!(
        props2.block_count > 0,
        "Table 2 should have at least one block"
    );
    assert_eq!(
        props2.seqnos,
        (1, 200),
        "Table 2 sequence numbers should be (1, 200)"
    );
    ...
    assert!(
        props3.data_size > 0,
        "Table 3 data size should be greater than 0"
    );
    assert!(props3.created_at > 0, "Table 3 created_at should be set");
    assert_eq!(
        props3.item_count, 300,
        "Table 3 item count should match entries"
    );
    assert_eq!(
        props3.key_count, 300,
        "Table 3 key count should match entries"
    );
    assert!(
        props3.block_count > 0,
        "Table 3 should have at least one block"
    );
    assert_eq!(
        props3.seqnos,
        (1, 300),
        "Table 3 sequence numbers should be (1, 300)"
    );
    ...
    assert_eq!(
        props1.tombstone_count, 0,
        "Table 1 should have no tombstones"
    );
    assert_eq!(
        props2.tombstone_count, 0,
        "Table 2 should have no tombstones"
    );
    assert_eq!(
        props3.tombstone_count, 0,
        "Table 3 should have no tombstones"
    );
}
...
fn test_lsn_with_multiple_l0_tables() {
    ...
    manifest
        .apply_changeset(&changeset1)
        .expect("Failed to apply changeset");

    assert_eq!(
        manifest.get_last_sequence(),
        10,
        "Single table should update last_sequence"
    );
...
fn test_manifest_v1_with_log_number_and_last_sequence() {
    ...
    assert_eq!(
        loaded_manifest.get_log_number(),
        42,
        "log_number should persist"
    );
    assert_eq!(
        loaded_manifest.get_last_sequence(),
        200,
        "last_sequence should persist"
    );
...
fn test_revert_empty_changeset() {
    ...
    assert_eq!(
        ...
        initial_log_number,
        "log_number should be unchanged"
    );
    assert_eq!(
...
fn test_revert_deleted_tables_only() {
    ...
    let rollback = manifest
        .apply_changeset(&delete_changeset)
        .expect("Failed to apply changeset");
    ...
    assert!(manifest.levels.get_levels()[0]
        .tables
        .iter()
        .any(|t| t.id == 1));
    assert!(manifest.levels.get_levels()[0]
        ...
        .iter()
        .any(|t| t.id == 2));
    assert!(manifest.levels.get_levels()[1]
        .tables
        .iter()
        .any(|t| t.id == 3));
}
...
fn test_revert_mixed_add_delete() {
    ...
    let rollback = manifest
        .apply_changeset(&mixed_changeset)
        .expect("Failed to apply changeset");
...
fn test_revert_preserves_table_ordering_l0() {
    ...
    let initial_order: Vec<u64> = manifest.levels.get_levels()[0]
        .tables
        .iter()
        .map(|t| t.id)
        .collect();
    ...
    let restored_order: Vec<u64> = manifest.levels.get_levels()[0]
        .tables
        .iter()
        .map(|t| t.id)
        .collect();
    ...
    assert_eq!(
        initial_order, restored_order,
        "L0 table ordering should be preserved"
    );
}
...
fn test_revert_preserves_table_ordering_l1() {
    ...
    // Capture initial ordering
    let initial_order: Vec<u64> = manifest.levels.get_levels()[1]
        .tables
        .iter()
        .map(|t| t.id)
        .collect();
    ...
    // Verify ordering is preserved
    let restored_order: Vec<u64> = manifest.levels.get_levels()[1]
        .tables
        .iter()
        .map(|t| t.id)
        .collect();
    ...
    assert_eq!(
        initial_order, restored_order,
        "L1+ table ordering should be preserved"
    );
}
...
fn test_revert_log_number_not_changed() {
    ...
    assert_eq!(
        manifest.get_log_number(),
        current_log_number,
        "log_number should not decrease"
    );
...
fn test_revert_last_sequence() {
    ...
    assert_eq!(
        manifest.get_last_sequence(),
        100,
        "last_sequence should be updated"
    );
...
fn test_revert_idempotent() {
    ...
    assert_eq!(
        state_after_revert, 0,
        "Table should be removed after revert"
    );
}
...
fn test_revert_after_disk_write_failure_simulation() {
    ...
    let _ = manifest
        .apply_changeset(&initial_changeset)
        .expect("Failed to apply changeset");
    ...
    let rollback = manifest
        .apply_changeset(&compaction_changeset)
        .expect("Failed to apply changeset");
    ...
    assert_eq!(
        manifest.get_last_sequence(),
        state_before.2,
        "last_sequence should be restored"
    );
    assert_eq!(
        manifest.get_log_number(),
        state_before.3,
        "log_number should be restored"
    );
    assert_eq!(
