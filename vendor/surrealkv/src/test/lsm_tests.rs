...
use crate::{
    CompressionType, Error, InternalKeyKind, Key, LSMIterator, Options, Tree, TreeBuilder, Value,
    WalRecoveryMode,
};
...
fn vlog_file_ids(path: &std::path::Path, opts: &Options) -> std::collections::BTreeSet<u32> {
    ...
    if let Ok(entries) = std::fs::read_dir(vlog_dir) {
        for entry in entries.flatten() {
            let name = entry.file_name().to_string_lossy().to_string();
            if let Some(id) = opts.extract_vlog_file_id(&name) {
                ids.insert(id);
            }
        }
    }
...
#[test(tokio::test)]
async fn test_large_single_commit_spans_multiple_memtables() {
    let temp_dir = create_temp_directory();
    let path = temp_dir.path().to_path_buf();

    let opts = create_test_options(path.clone(), |opts| {
        opts.max_memtable_size = 2 * 1024; // Force a single tx commit to rotate memtables
    ...
    for i in 0..256 {
        ...
        tx.set(key.as_bytes(), &value).unwrap();
    }
    tx.commit().await.unwrap();

    let read_tx = tree.begin().unwrap();
    for i in 0..256 {
        let key = format!("k{:04}", i);
...
async fn test_memtable_flush_with_multiple_keys_and_updates() {
    ...
    let l0_size = tree.core.level_manifest.read().unwrap().levels.get_levels()[0]
        .tables
        .len();
...
async fn test_range_limit_functionality() {
    ...
    assert_eq!(
        single_result.len(),
        1,
        "Range scan with .take(1) should return exactly 1 item"
    );
    ...
    assert_eq!(
        zero_result.len(),
        0,
        "Range scan with .take(0) should return no items"
    );
}
    ...
    // Step 1: Create database and write data for first table
    {
        ...
        for batch in 0..2 {
            for i in 0..6 {
                ...
                assert!(
                    result.is_some(),
                    "Key '{}' should exist immediately after flush",
                    key
                );
...
async fn test_table_id_assignment_across_restart() {
    ...
    // Step 1: Create initial database and write data for 2 memtables
    {
        ...
        assert_eq!(
            l0_size, 2,
            "Expected 2 tables in L0 after initial writes, got {l0_size}"
        );
    ...
    // Step 2: Reopen the database
    {
...

        {
            ...
            assert_eq!(
                l0_size, 2,
                "Expected 2 tables in L0 after reopening, got {l0_size}"
            );
    ...
    // Step 4: Final verification - reopen and check everything is still correct
    {
        ...
        assert_eq!(
            l0_size, 3,
            "Expected 3 tables in L0 after final reopen, got {l0_size}"
        );
...
async fn test_vlog_prefill_on_reopen() {
    ...
    for i in 0..10 {
        ...
        assert!(
            result.is_some(),
            "Key '{key}' should exist immediately after write"
        );
    }
    ...
    for i in 0..10 {
        ...
        assert!(
            result.is_some(),
            "Key '{key}' should exist in first database"
        );
        assert_eq!(&result.unwrap(), expected_value.as_bytes());
    }
    ...
    assert!(
        tree2.core.vlog.is_some(),
        "VLog should be enabled in second database"
    );
...
async fn test_soft_delete() {
    ...
    // Step 2: Reopen the database and verify soft deleted key is still invisible
    {
        ...
        let mut txn = tree.begin().unwrap();
        txn.set(b"test_key", b"new_value_after_soft_delete")
            .unwrap();
        txn.commit().await.unwrap();
...
async fn test_checkpoint_with_vlog() {
    ...
    assert!(
        !vlog_files.is_empty(),
        "Should have VLog files in the checkpoint"
    );
    ...
    assert!(
        vlog_dir.exists(),
        "VLog directory should exist after restore"
    );
    ...
    assert!(
        !vlog_files.is_empty(),
        "Should have VLog files after restore"
    );
}
...
async fn test_clean_shutdown_actually_skips_wal() {
    ...
    // Phase 3: Restart and verify WAL was actually skipped
    {
        ...
        assert!(
            memtable_before.is_empty(),
            "Memtable should be empty before WAL replay"
        );
...
async fn test_log_number_advances_with_flushes() {
    ...
    let log_number_1 = tree
        ...
        .get_log_number();
    assert!(
        log_number_1 > log_number_0,
        "log_number should advance after flush"
    );
    ...
    let log_number_2 = tree
        ...
        .get_log_number();
    assert!(
        log_number_2 > log_number_1,
        "log_number should advance after second flush"
    );
...
async fn test_last_sequence_persists_across_restart() {
    ...
    // Phase 1: Create database, write, flush
    {
        ...
        assert!(
            expected_last_seq > 0,
            "last_sequence should be > 0 after flush"
        );
...
async fn test_wal_recovery_updates_last_sequence_in_memory() {
    ...
    // Phase 3: Recover and verify in-memory sequence > manifest sequence
    {
        ...
        tree.flush().unwrap();
        let manifest_seq_after_flush = tree
            .core
            .inner
            .level_manifest
            .read()
...
async fn test_multiple_flush_cycles_log_number_sequence() {
...

    {
        ...
        for i in 0..50 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("batch1_key_{i}").as_bytes(), b"value1")
                .unwrap();
            txn.commit().await.unwrap();
        }
        tree.flush().unwrap();
        ...
        for i in 0..50 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("batch2_key_{i}").as_bytes(), b"value2")
                .unwrap();
            txn.commit().await.unwrap();
        }
        tree.flush().unwrap();
        ...
        for i in 0..50 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("batch3_key_{i}").as_bytes(), b"value3")
                .unwrap();
            txn.commit().await.unwrap();
        }
        tree.flush().unwrap();
...
async fn test_full_crash_recovery_scenario() {
    ...
    // Phase 1: Write batch A, flush
    {
        ...
        for i in 0..50 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("batch_a_{i}").as_bytes(), b"value_a")
                .unwrap();
            txn.commit().await.unwrap();
        }
        tree.flush().unwrap();
    ...
    // Phase 2: Write batch B, flush
    {
        ...
        for i in 0..50 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("batch_b_{i}").as_bytes(), b"value_b")
                .unwrap();
            txn.commit().await.unwrap();
        }
        tree.flush().unwrap();
        ...
        let log_number_after_b = tree
            ...
            .get_log_number();
        assert!(
            log_number_after_b >= 2,
            "Should have rotated WAL at least twice"
        );
    ...
    // Phase 3: Write batch C, crash before flush
    {
        ...
        for i in 0..20 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("batch_c_{i}").as_bytes(), b"value_c")
                .unwrap();
            txn.commit().await.unwrap();
        }
...
async fn test_rotate_memtable_waits_on_manifest_before_immutable_lock() {
    let temp_dir = TempDir::new("test").unwrap();
    let path = temp_dir.path().to_path_buf();
    ...
    // Ensure active memtable is non-empty so rotate_memtable() performs real work.
    let mut txn = tree.begin().unwrap();
    txn.set(b"lock_order_key", b"value").unwrap();
    txn.commit().await.unwrap();

    // Hold manifest write lock to block rotate_memtable() at manifest access.
    let manifest_guard = tree.core.inner.level_manifest.write().unwrap();
    let inner = Arc::clone(&tree.core.inner);

    let rotate_handle = std::thread::spawn(move || inner.rotate_memtable());

    // Give rotate thread time to attempt lock acquisition.
    tokio::time::sleep(tokio::time::Duration::from_millis(50)).await;

    // If rotate_memtable acquires immutable first, this would fail (old deadlock-prone order).
    let immutable_guard = tree.core.inner.immutable_memtables.try_write();
    assert!(
        immutable_guard.is_ok(),
        "rotate_memtable() should wait on manifest before taking immutable_memtables lock"
    );
    drop(immutable_guard);

    // Unblock rotate and ensure it completes.
    drop(manifest_guard);
    let rotate_result = tokio::task::spawn_blocking(move || rotate_handle.join())
        .await
        .unwrap()
        .unwrap();
    assert!(
        rotate_result.is_ok(),
        "rotate_memtable should complete successfully"
    );
...
async fn test_multiple_flush_cycles_with_sst_and_wal_verification() {
    ...
    // Cycle 1: Write data, trigger flush, close
    {
        ...
        for i in 0..50 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("cycle1_key_{}", i).as_bytes(), b"value1")
                .unwrap();
            txn.commit().await.unwrap();
        }
    ...
    // Cycle 2: Reopen, verify recovery, write more, flush, close
    {
        ...
        for i in 0..50 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("cycle2_key_{}", i).as_bytes(), b"value2")
                .unwrap();
            txn.commit().await.unwrap();
        }
...

        {
            ...
            assert!(
                ...
                "Second flush should create more SST"
            );
            assert!(
                manifest.get_log_number() > log_num_before,
                "log_number should advance"
            );
            drop(manifest);
        }
    ...
    // Cycle 3: Reopen, write but DON'T flush, close (tests shutdown flush)
    {
        ...
        for i in 0..50 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("cycle3_key_{}", i).as_bytes(), b"value3")
                .unwrap();
            txn.commit().await.unwrap();
        }
...
async fn test_flush_on_close_option_comparison() {
    // Test with flush_on_close = true
    {
        ...
        assert_eq!(
            sst_after,
            sst_before + 1,
            "flush_on_close=true should create SST"
        );
        tree.close().await.unwrap();
    }
    ...
    // Test with flush_on_close = false
    {
        ...
        assert_eq!(
            sst_after, sst_before,
            "flush_on_close=false should NOT create SST"
        );
...
async fn test_flush_all_memtables_on_close_ordering() {
...

    {
        ...
        for i in 0..50 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("key_{:04}", i).as_bytes(), b"value_data_here")
                .unwrap();
            txn.commit().await.unwrap();
        }
    ...
    // Reopen and verify all data is accessible
    {
        ...
        for i in 0..50 {
            ...
            assert!(
                value.is_some(),
                "Key {} should exist after close/reopen",
                key
            );
        }
...
async fn test_flush_immutable_memtables_with_empty_active() {
...

    {
        ...
        for i in 0..20 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("key_{}", i).as_bytes(), b"value_data")
                .unwrap();
            txn.commit().await.unwrap();
        }
...
async fn test_wal_number_tracking_on_flush() {
    ...
    let after_second_flush_log = tree
        .core
        .inner
        .level_manifest
        .read()
...
async fn test_memtable_wal_number_after_swap() {
    ...
    assert!(
        wal_2 > wal_1,
        "WAL number should increase after flush: {} > {}",
        wal_2,
        wal_1
    );
    ...
    for cycle in 1..=3 {
        {
            ...
            for i in 0..100 {
                let mut txn = tree.begin().unwrap();
                txn.set(format!("cycle{}_key_{}", cycle, i).as_bytes(), b"value")
                    .unwrap();
                txn.commit().await.unwrap();
            }
        ...
        // Verify data from this cycle is accessible
        {
            ...
            for prev_cycle in 1..cycle {
                assert_eq!(
                    txn.get(format!("cycle{}_key_0", prev_cycle).as_bytes())
                        .unwrap(),
                    Some(b"value".to_vec()),
...
async fn test_crash_recovery_with_orphaned_sst() {
    ...
    // Phase 3: Reopen - should cleanup orphaned SST and recover from WAL
    {
        ...
        assert!(
            !orphaned_sst_path.exists(),
            "Orphaned SST should be removed"
        );
...
async fn test_wal_recovery_mode_tolerate_with_repair_succeeds_on_corruption() {
    ...
    // Phase 3: Open with default TolerateCorruptedWithRepair mode - should SUCCEED
    {
        ...
        let tree = match result {
            ...
            Err(e) => {
                panic!(
                    "TolerateCorruptedWithRepair should succeed after repairing WAL: {}",
                    e
                )
            }
        };
...
async fn test_recovery_with_manually_created_wal_segments() {
    ...
    let active_wal_after_recovery;
    {
        ...
        txn.commit().await.unwrap();
        log::info!(
            "Phase 3: Wrote key4 to WAL segment {}",
            active_wal_after_recovery
        );

        // Flush - this updates log_number
        tree.flush().unwrap();
        let log_number_after_flush = tree
            .core
            .inner
            .level_manifest
            .read()
            ...
            .get_log_number();
        log::info!(
            "Phase 3: After flush, log_number={}",
            log_number_after_flush
        );
    ...
    for _ in 0..num_flush_tasks {
        ...
        join_set.spawn(async move {
            for i in 0..flushes_per_task {
                ...
                assert!(
                    result.is_ok(),
                    "flush_wal should succeed during concurrent commits"
                );
    ...
    // Verify all data is accessible
    {
        ...
        for task_id in 0..num_commit_tasks {
            for i in 0..commits_per_task {
                ...
                assert!(
                    result.is_some(),
                    "Key {} should exist after concurrent operations",
                    key
                );
            }
        }
...
async fn test_range_exclusive_boundaries() {
    ...
    for key in keys.iter() {
        let mut txn = tree.begin().unwrap();
        txn.set(key.as_bytes(), format!("value_{}", key).as_bytes())
            .unwrap();
        txn.commit().await.unwrap();
    }
    ...
    // Test before flush
    {
        ...
        assert_eq!(
            range_result.len(),
            2,
            "Range [a, c) should return 2 keys (a and b)"
        );
        ...
        assert!(
            ...
            "Range [a, c) should include 'a'"
        );
        assert!(
            result_keys.contains(&"b".to_string()),
            "Range [a, c) should include 'b'"
        );
        assert!(
        ...
        assert!(
            !result_keys.contains(&"d".to_string()),
            "Range [a, c) should NOT include 'd'"
        );
        ...
        assert_eq!(
            range_result2.len(),
            2,
            "Range [b, d) should return 2 keys (b and c)"
        );
        ...
        assert!(
            !result_keys2.contains(&"a".to_string()),
            "Range [b, d) should NOT include 'a'"
        );
        assert!(
            result_keys2.contains(&"b".to_string()),
            "Range [b, d) should include 'b'"
        );
        assert!(
            result_keys2.contains(&"c".to_string()),
            "Range [b, d) should include 'c'"
        );
        assert!(
            !result_keys2.contains(&"d".to_string()),
            "Range [b, d) should NOT include 'd'"
        );
        ...
        assert_eq!(
            range_result3.len(),
            2,
            "Range [c, e) should return 2 keys (c and d)"
        );
        ...
        assert!(
            result_keys3.contains(&"c".to_string()),
            "Range [c, e) should include 'c'"
        );
        assert!(
            result_keys3.contains(&"d".to_string()),
            "Range [c, e) should include 'd'"
        );
    }
...

    {
        ...
        assert_eq!(
            range_result.len(),
            2,
            "After flush: Range [a, c) should return 2 keys"
        );
...
async fn test_range_boundary_edge_cases() {
    ...
    // Test before flush
    {
        ...
        assert!(
            result_keys.contains(&"aa".to_string()),
            "Should include 'aa'"
        );
        assert!(
            result_keys.contains(&"aaa".to_string()),
            "Should include 'aaa'"
        );
        assert!(
            result_keys.contains(&"aaaa".to_string()),
            "Should include 'aaaa'"
        );
        assert!(
            result_keys.contains(&"aaaab".to_string()),
            "Should include 'aaaab'"
        );
        assert!(
            !result_keys.contains(&"ab".to_string()),
            "Should NOT include 'ab' (upper bound)"
        );
        assert!(
            !result_keys.contains(&"b".to_string()),
        ...
        assert!(
            exact_keys.contains(&"aaaa".to_string()),
            "Should include 'aaaa'"
        );
        assert!(
            exact_keys.contains(&"aaaab".to_string()),
            "Should include 'aaaab'"
        );
    }
...

    {
        ...
        assert_eq!(
            result_keys[0], "aa",
            "After flush: First key should be 'aa'"
        );
        assert_eq!(
            result_keys[1], "aaa",
            "After flush: Second key should be 'aaa'"
        );
        assert_eq!(
            result_keys[2], "aaaa",
            "After flush: Third key should be 'aaaa'"
        );
        assert_eq!(
            result_keys[3], "aaaab",
            "After flush: Fourth key should be 'aaaab'"
        );
    }
}
...
#[test(tokio::test)]
async fn test_incremental_vlog_gc_reclaims_partially_garbage_file() {
    let temp_dir = create_temp_directory();
    let path = temp_dir.path().to_path_buf();
    ...
    let opts = create_test_options(path.clone(), |opts| {
        ...
        opts.vlog_max_file_size = 1024 * 1024;
    });

    let tree = Tree::new(Arc::clone(&opts)).unwrap();

    let key_count = 64usize;
    let value_size = 8 * 1024usize;
    for i in 0..key_count {
        let mut tx = tree.begin().unwrap();
        let key = format!("gc-key-{i:04}");
        let value = format!("seed-{i:04}-{}", "A".repeat(value_size));
        tx.set(key.as_bytes(), value.as_bytes()).unwrap();
        tx.commit().await.unwrap();
    }
    tree.flush().unwrap();

    let ids_after_seed = vlog_file_ids(&path, &opts);
    assert!(
        ids_after_seed.contains(&1),
        "Expected first VLog file (id=1) after seed phase, got {:?}",
        ids_after_seed
    );
    ...
    for round in 0..6 {
        for i in 1..key_count {
            let mut tx = tree.begin().unwrap();
            let key = format!("gc-key-{i:04}");
            let value = format!("update-r{round}-{i:04}-{}", "B".repeat(value_size));
            tx.set(key.as_bytes(), value.as_bytes()).unwrap();
            tx.commit().await.unwrap();
        }
    ...
    let ids_after_gc = vlog_file_ids(&path, &opts);
    assert!(
        !ids_after_gc.contains(&1),
        "Expected VLog file id=1 to be reclaimed after relocation GC, remaining files: {:?}",
        ids_after_gc
    );
    ...
    for i in 0..key_count {
        ...
        if i == 0 {
            let expected = format!("seed-{i:04}-{}", "A".repeat(value_size));
            assert_eq!(
                got,
                Some(expected.into_bytes()),
                "untouched key must stay readable"
            );
        } else {
            let expected = format!("update-r{}-{i:04}-{}", 5, "B".repeat(value_size));
            assert_eq!(
                got,
                Some(expected.into_bytes()),
                "updated key must return latest value"
            );
        }
    }
}
...
#[test(tokio::test)]
async fn test_incremental_vlog_gc_respects_per_compaction_budget() {
    let temp_dir = create_temp_directory();
    let path = temp_dir.path().to_path_buf();
    ...
    let stable_prefix = 10usize;
    let value_size = 8 * 1024usize;

    for i in 0..key_count {
        let mut tx = tree.begin().unwrap();
        let key = format!("budget-key-{i:04}");
        let value = format!("seed-{i:04}-{}", "S".repeat(value_size));
        tx.set(key.as_bytes(), value.as_bytes()).unwrap();
        tx.commit().await.unwrap();
    }
    tree.flush().unwrap();

    for i in stable_prefix..key_count {
        let mut tx = tree.begin().unwrap();
        let key = format!("budget-key-{i:04}");
        let value = format!("new-{i:04}-{}", "N".repeat(value_size));
        tx.set(key.as_bytes(), value.as_bytes()).unwrap();
        tx.commit().await.unwrap();
    }
    tree.flush().unwrap();

    let strategy: Arc<dyn crate::compaction::CompactionStrategy> = Arc::new(Strategy::default());
    tree.core.compact(strategy).unwrap();

    let ids_after_one_compaction = vlog_file_ids(&path, &opts);
    assert!(
		ids_after_one_compaction.contains(&1),
		"With tight per-compaction rewrite budget, file id=1 should still exist after one compaction; got {:?}",
		ids_after_one_compaction
	);

    for round in 0..8 {
        let mut tx = tree.begin().unwrap();
        let key = format!("budget-dummy-{round:04}");
        let value = format!("dummy-{round}-{}", "D".repeat(value_size));
        tx.set(key.as_bytes(), value.as_bytes()).unwrap();
        tx.commit().await.unwrap();
        tree.flush().unwrap();
    ...
    let ids_after_progress = vlog_file_ids(&path, &opts);
    assert!(
        ids_after_progress.contains(&1),
        "Tight budget should avoid aggressive one-shot relocation of file id=1; files: {:?}",
        ids_after_progress
    );
    ...
    for i in 0..key_count {
        ...
        let tx = tree.begin().unwrap();
        let got = tx.get(key.as_bytes()).unwrap();
        let expected = if i < stable_prefix {
            format!("seed-{i:04}-{}", "S".repeat(value_size))
        } else {
            format!("new-{i:04}-{}", "N".repeat(value_size))
        };
        assert_eq!(
            got,
            Some(expected.into_bytes()),
...
async fn test_vlog_files_persist_across_restart() {
    ...
    let vlog_files_before_shutdown: Vec<String>;
    {
        ...
        log::info!(
            "VLog files before shutdown: {} files",
            vlog_files_before_shutdown.len()
        );
    ...
    for iteration in 1..=5 {
        ...
        log::info!(
            "Added 10 new entries (total records: {})",
            keys_and_values.len()
        );
...
async fn test_vlog_gc_non_versioned_surrealdb_style_keys() {
    ...
    fn print_storage_state(path: &std::path::Path, tree: &Tree, phase: &str, round: usize) {
        ...
        println!(
            "[{} round {}] SSTables per level: {:?}",
            phase, round, level_counts
        );
        println!("---");
    }
