...
async fn test_hard_delete_at_non_bottom_level() {
    ...
    assert_eq!(
        returned_value.len(),
        0,
        "hard_delete should have empty value"
    );
}
...
async fn test_multiple_keys_with_mixed_scenarios() {
    ...
    assert_eq!(
        result[1].1.len(),
        0,
        "key2 hard_delete should have empty value"
    );
    assert_eq!(
...
async fn test_sequence_ordering_across_iterators() {
    ...
    let items2 = vec![
        (key_a_v2, val_a_v2),
        (key_b_v2, val_b_v2.clone()),
        (key_c_v2, val_c_v2),
    ];
    let items3 = vec![
        (key_a_v1, val_a_v1),
        (key_c_v1, val_c_v1),
        (key_d_v1, val_d_v1.clone()),
    ];
    ...
    assert_eq!(
        result[2].1.len(),
        0,
        "key_c hard_delete should have empty value"
    );
    assert_eq!(
        result[3].1, val_d_v1,
...
async fn test_compaction_iterator_versioning_retention_logic() {
    ...
    assert_eq!(
        result.len(),
        7,
        "Expected 7 items: 3 from key1, 3 from key2, 1 from key3"
    );

    // Verify we get all versions of key1 (all within retention)
    let key1_versions: Vec<_> = result
        .iter()
        .filter(|(k, _)| &k.user_key == b"key1")
        .collect();
    assert_eq!(
        key1_versions.len(),
        3,
        "key1 should have 3 versions within retention"
    );

    // Verify we get all versions of key2 (all within retention)
    let key2_versions: Vec<_> = result
        .iter()
        .filter(|(k, _)| &k.user_key == b"key2")
        .collect();
    assert_eq!(
        key2_versions.len(),
        3,
        "key2 should have 3 versions within retention"
    );

    // Verify we get only the recent version of key3 (very old versions outside
    // retention)
    let key3_versions: Vec<_> = result
        .iter()
        .filter(|(k, _)| &k.user_key == b"key3")
        .collect();
    assert_eq!(
        key3_versions.len(),
        1,
...
async fn test_compaction_iterator_versioning_retention_bottom_level() {
    ...
    assert_eq!(
        result.len(),
        1,
        "Expected 1 item: only key3 with recent SET"
    );
    ...
    let key4_versions: Vec<_> = result
        .iter()
        .filter(|(k, _)| &k.user_key == b"key4")
        .collect();
    assert_eq!(
        key4_versions.len(),
        0,
...
async fn test_compaction_iterator_no_versioning_non_bottom_level() {
    ...
    assert_eq!(
        result.len(),
        3,
        "Expected 3 items: latest version of each key"
    );
    ...
    assert_eq!(
        key3_versions.len(),
        1,
        "key3 should have only 1 version (DELETE)"
    );
...
async fn test_compaction_iterator_no_versioning_bottom_level() {
    ...
    assert_eq!(
        result.len(),
        1,
        "Expected 1 item: only key1 with latest SET"
    );
...
fn test_merging_iterator_direction_switch_with_duplicate_keys() {
    ...
    let iter0 = build_table_iterator(vec![
        (
            create_internal_key("key-1", 100, InternalKeyKind::Set),
            Value::from(b"v1".to_vec()),
        ),
        (
            create_internal_key("key-3", 200, InternalKeyKind::Set),
            Value::from(b"v3-L0".to_vec()),
        ),
        (
            create_internal_key("key-5", 100, InternalKeyKind::Set),
            Value::from(b"v5".to_vec()),
        ),
    ]);

    let iter1 = build_table_iterator(vec![
        (
            create_internal_key("key-2", 100, InternalKeyKind::Set),
            Value::from(b"v2".to_vec()),
        ),
        (
            create_internal_key("key-3", 100, InternalKeyKind::Set),
            Value::from(b"v3-L1".to_vec()),
        ),
        (
            create_internal_key("key-6", 100, InternalKeyKind::Set),
            Value::from(b"v6".to_vec()),
        ),
    ]);
...
fn test_snapshot_compaction_no_snapshots_keeps_only_latest() {
    ...
    let items = vec![
        (
            create_internal_key("key1", 100, InternalKeyKind::Set),
            b"v3".to_vec(),
        ),
        (
            create_internal_key("key1", 50, InternalKeyKind::Set),
            b"v2".to_vec(),
        ),
        (
            create_internal_key("key1", 20, InternalKeyKind::Set),
            b"v1".to_vec(),
        ),
    ];
    ...
    assert_eq!(
        result.len(),
        1,
        "Only latest version should be kept with no snapshots"
    );
...
fn test_snapshot_compaction_single_snapshot_preserves_visible_version() {
    ...
    let items = vec![
        ...
        (
            create_internal_key("key1", 30, InternalKeyKind::Set),
            b"v2".to_vec(),
        ),
        (
            create_internal_key("key1", 10, InternalKeyKind::Set),
            b"v1".to_vec(),
        ),
    ];
    ...
    assert_eq!(
        result.len(),
        2,
        "Latest and snapshot-visible version should be kept"
    );
    assert!(
        result.contains(&("key1".to_string(), 100)),
        "Latest should be kept"
    );
    assert!(
        result.contains(&("key1".to_string(), 30)),
        "Snapshot-visible version should be kept"
    );
}
...
fn test_snapshot_compaction_multiple_snapshots_different_boundaries() {
    ...
    let items = vec![
        (
            create_internal_key("key1", 200, InternalKeyKind::Set),
            b"v4".to_vec(),
        ), /* Newer than
            * all snapshots */
        (
            create_internal_key("key1", 120, InternalKeyKind::Set),
            b"v3".to_vec(),
        ), /* Visible to
            * snap 150 */
        (
            create_internal_key("key1", 80, InternalKeyKind::Set),
            b"v2".to_vec(),
        ), /* Visible to
            * snap 150 (same
            * boundary as
    ...
    assert_eq!(
        result.len(),
        3,
        "Three versions should be kept (one per boundary)"
    );
    assert!(
        result.contains(&("key1".to_string(), 200)),
        "Latest should be kept"
    );
    assert!(
...
fn test_snapshot_compaction_newer_version_hides_older_in_same_boundary() {
    ...
    assert_eq!(
        result.len(),
        1,
        "Only newer version in same boundary should be kept"
    );
    assert_eq!(result[0], ("key1".to_string(), 100));
}
...
fn test_snapshot_compaction_versions_at_tip_hidden_by_newer() {
    ...
    let items = vec![
        (
            create_internal_key("key1", 300, InternalKeyKind::Set),
            b"v3".to_vec(),
        ),
        (
            create_internal_key("key1", 200, InternalKeyKind::Set),
        ...
        (
            create_internal_key("key1", 150, InternalKeyKind::Set),
            b"v1".to_vec(),
        ),
    ];
    ...
    assert_eq!(
        result.len(),
        1,
        "Only latest version should be kept when all newer than snapshots"
    );
...
fn test_snapshot_compaction_multiple_keys() {
    let items = vec![
        ...
        (
            create_internal_key("key2", 80, InternalKeyKind::Set),
            b"k2v2".to_vec(),
        ),
        (
            create_internal_key("key2", 40, InternalKeyKind::Set),
            b"k2v1".to_vec(),
        ),
    ];
    ...
    assert_eq!(
        result.len(),
        4,
        "All versions in different boundaries should be kept"
    );
}
...
fn test_snapshot_compaction_tombstone_visible_to_snapshot() {
    ...
    let items = vec![
        (
            create_internal_key("key1", 100, InternalKeyKind::Delete),
            b"".to_vec(),
        ),
        (
            create_internal_key("key1", 50, InternalKeyKind::Set),
    ...
    assert_eq!(
        result[0],
        ("key1".to_string(), 100, InternalKeyKind::Delete)
    );
}
...
fn test_snapshot_compaction_tombstone_at_bottom_with_snapshot() {
    ...
    assert_eq!(
        result.len(),
        0,
        "Bottom level delete should drop all versions"
    );
}
...
fn test_snapshot_compaction_exact_sequence_match() {
    ...
    assert_eq!(
        result.len(),
        2,
        "Both versions should be kept with exact snapshot match"
    );
...
fn test_snapshot_compaction_with_versioning_enabled() {
    ...
    let items = vec![
        ...
        (
            create_internal_key_with_timestamp("key1", 50, InternalKeyKind::Set, 800),
            b"v2".to_vec(),
        ),
        (
            create_internal_key_with_timestamp("key1", 20, InternalKeyKind::Set, 500),
            b"v1".to_vec(),
        ),
    ];
    ...
    assert_eq!(
        result.len(),
        2,
        "Latest and snapshot-visible should be kept"
    );
    assert!(result.contains(&("key1".to_string(), 100)));
    assert!(result.contains(&("key1".to_string(), 50)));
}
