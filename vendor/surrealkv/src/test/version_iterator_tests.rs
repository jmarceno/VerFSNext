...
async fn test_history_with_tombstones() {
    ...
    assert_eq!(
        results.len(),
        1,
        "Only key2 visible - hard delete hides all"
    );
...
async fn test_history_replace_shows_all_versions() {
    ...
    assert_eq!(
        results.len(),
        3,
        "All 3 replace operations should be visible"
    );
...
async fn test_history_seek_middle() {
    ...
    for i in 1..=5 {
        ...
        tx.set_at(key.as_bytes(), value.as_bytes(), i as u64 * 100)
            .unwrap();
        tx.commit().await.unwrap();
    }
...
async fn test_history_requires_versioning() {
    ...
    assert!(
        result.is_err(),
        "history should fail without versioning enabled"
    );
}
...
async fn test_history_transaction_states() {
    ...
    // Test on write-only transaction
    {
        ...
        assert!(
            result.is_err(),
            "Write-only transaction should not allow history"
        );
    }
}
...
async fn test_replace_cuts_off_history_with_versioning() {
    ...
    // Before flush - REPLACE filters immediately during iteration (not after compaction)
    {
        ...
        assert_eq!(
            results.len(),
            1,
            "Before flush: REPLACE filters immediately"
        );
    ...
    assert_eq!(
        results.len(),
        1,
        "After flush: only Replace version survives"
    );
    assert_eq!(results[0].1, b"replace_v3");
    assert_eq!(results[0].2, 300);
}
...
async fn test_versions_survive_compaction() {
    ...
    assert_eq!(
        results.len(),
        10,
        "All 10 versions should survive multiple flushes"
    );
...
async fn test_history_ts_range_forward() {
    ...
    assert_eq!(
        results.len(),
        2,
        "Should have 2 versions in range [150, 350]"
    );
    assert_eq!(results[0].2, 300); // timestamp
    assert_eq!(results[1].2, 200);
}
...
async fn test_history_ts_range_backward() {
    ...
    assert_eq!(
        results.len(),
        1,
        "Should have 1 version in range [150, 250]"
    );
...
async fn test_history_ts_range_btree() {
    ...
    assert_eq!(
        results.len(),
        2,
        "Should have 2 versions in range [100, 200]"
    );
    assert_eq!(results[0].2, 200);
...
async fn test_history_limit_forward() {
    ...
    for i in 1..=5 {
        let mut tx = store.begin().unwrap();
        tx.set_at(b"key1", format!("v{}", i).as_bytes(), i * 100)
            .unwrap();
        tx.commit().await.unwrap();
    }
    ...
    assert_eq!(
        results.len(),
        3,
        "Should have exactly 3 entries due to limit"
    );
...
async fn test_history_limit_backward() {
    ...
    assert_eq!(
        results.len(),
        2,
        "Should have exactly 2 entries due to limit"
    );
...
async fn test_history_limit_multiple_keys() {
    ...
    assert_eq!(
        results.len(),
        3,
        "Should have exactly 3 entries across keys"
    );
}
...
async fn test_get_at_ryow() {
    ...
    assert_eq!(
        value,
        Some(b"uncommitted_v2".to_vec()),
        "Should see uncommitted write at ts=200"
    );
    ...
    assert_eq!(
        value,
        Some(b"uncommitted_new".to_vec()),
        "Should see uncommitted new key"
    );
    ...
    assert_eq!(
        value,
        Some(b"committed_v1".to_vec()),
        "Should see committed data at ts=100"
    );
}
...
async fn test_get_at_ryow_future_timestamp() {
    ...
    assert_eq!(
        value, None,
        "Should not see future write at ts=400 < write ts=500"
    );
    ...
    assert_eq!(
        value,
        Some(b"future_value".to_vec()),
        "Should see write at ts=500"
    );
    ...
    assert_eq!(
        value,
        Some(b"future_value".to_vec()),
        "Should see write at ts=600 > write ts=500"
    );
}
...
async fn test_history_ryow_timestamp_collision() {
    ...
    assert_eq!(
        results.len(),
        1,
        "Should have 1 entry (uncommitted shadows committed at same ts)"
    );
    assert_eq!(results[0].0, b"key1".to_vec());
...
async fn test_history_ryow_with_ts_range() {
    ...
    assert_eq!(
        results.len(),
        3,
        "Should have 3 entries in range [100, 200]"
    );
    assert_eq!(results[0].2, 200);
...
async fn test_history_ryow_soft_delete() {
    ...
    assert_eq!(
        results.len(),
        1,
        "Should have 1 entry (soft delete filtered)"
    );
    ...
    assert!(
        results[0].3,
        "First entry should be tombstone (is_tombstone=true)"
    );
...
async fn test_history_ryow_hard_delete() {
    ...
    assert_eq!(
        results.len(),
        1,
        "Should have 1 entry (key1 wiped by hard delete)"
    );
    ...
    assert_eq!(
        results.len(),
        1,
        "Hard delete wipes history even with tombstones=true"
    );
    assert_eq!(results[0].0, b"key2".to_vec());
}
