...
fn test_table_seek() {
    ...
    assert!(iter.valid());
    assert_eq!(
        (iter.key().user_key(), iter.value_encoded().unwrap()),
        (&b"bcd"[..], &b"asa"[..])
    );
...
fn add_key(writer: &mut TableWriter<Vec<u8>>, key: &[u8], seq: u64, value: &[u8]) -> Result<()> {
    writer.add(
        InternalKey::new(Vec::from(key), seq, InternalKeyKind::Set, 0),
        value,
    )
}
...
fn test_writer_key_range_with_special_chars() {
    ...
    if let Some(smallest) = &writer.meta.smallest_point {
        assert_eq!(
            &smallest.user_key[..],
            sorted_keys.first().unwrap().as_bytes()
        );
    }
    if let Some(largest) = &writer.meta.largest_point {
        assert_eq!(
            &largest.user_key[..],
            sorted_keys.last().unwrap().as_bytes()
        );
    }
}
...
fn test_table_iterator_advance_method_correctness() {
    ...
    assert!(
        !final_advance,
        "next() should return false when trying to advance past the last item"
    );
    assert!(
        !iter.valid(),
    ...
    for i in 0..3 {
        ...
        assert!(
            !iter.valid(),
            "Iterator should remain invalid after additional next() calls"
        );
    }
}
...
fn test_table_iterator_positioning_edge_cases() {
    ...
    // that shouldn't normally occur in production.
    {
        ...
        assert!(
            result.is_err(),
            "next() on empty table should return error (no valid blocks)"
        );
    ...
    // Test 3: Reset behavior after exhaustion
    {
        ...
        assert!(
            !iter.next().unwrap(),
            "next() after exhaustion should return false"
        );
        assert!(!iter.valid(), "Iterator should remain invalid");
...
fn test_table_iterator_next_vs_advance_consistency() {
    ...
    while iter1.valid() {
        collected_via_advance.push((
            iter1.key().to_owned(),
            iter1.value_encoded().unwrap().to_vec(),
        ));
    ...
    while iter2.valid() {
        collected_via_next.push((
            iter2.key().to_owned(),
            iter2.value_encoded().unwrap().to_vec(),
        ));
...
fn test_table_iterator_seek_nonexistent_key() {
    ...
    assert!(
        iter.valid(),
        "Iterator should be valid (positioned at next key)"
    );
    ...
    assert!(
        !is_exact_match,
        "Caller should check for exact match if needed"
    );
}
...
fn test_table_iterator_seek_nonexistent_past_end() {
    ...
    assert!(
        !iter.valid(),
        "Iterator should be invalid when seeking past all keys"
    );
}
...
fn test_table_iter_unbounded_reverse() {
    ...
    assert_eq!(
        collected.len(),
        10,
        "Forward iteration should return 10 items"
    );
...
fn test_table_iter_upper_bound_excluded_reverse() {
    ...
    assert!(
        !results.iter().any(|k| k == "key_005"),
        "key_005 should not appear in results"
    );
}
...
fn test_table_iter_upper_bound_included_reverse_nonexistent_key() {
    ...
    assert!(
        !results.iter().any(|k| k == "key_006"),
        "key_006 should not appear"
    );
}
...
fn test_table_iter_lower_bound_excluded_forward_with_multiple_versions() {
    ...
    assert!(
        results[0] == "key_004",
        "First result should be key_004, got {}",
        results[0]
    );
    ...
    assert!(
        ...
        "key_003 should not appear in results"
    );
}
...
fn test_table_iter_excluded_bound_across_multiple_blocks() {
    ...
    assert_eq!(
        results.len(),
        2,
        "Should return exactly 2 keys, got {}",
        results.len()
    );
}
...
fn test_table_iter_both_bounds_excluded_same_key_reverse() {
    ...
    while iter.valid() {
        results.push((
            iter.key().to_owned(),
            iter.value_encoded().unwrap().to_vec(),
        ));
        if !iter.prev().unwrap() {
            break;
        }
...
fn test_table_iter_excluded_bounds_adjacent_keys() {
    ...
    assert!(
        results.iter().all(|(k, _)| k == "key_003"),
        "All results should be key_003"
    );
...
fn test_partitioned_index_same_user_key_spanning_partitions_bug() {
    ...
    for seq in (1..=num_versions).rev() {
        ...
        writer
            .add(key, format!("value_at_seq_{}", seq).as_bytes())
            .unwrap();
    }
...
fn test_table_get_tombstone_handling() {
    ...
    assert_eq!(found_key.seq_num(), 50);
    assert!(
        found_key.is_tombstone(),
        "Should return the tombstone marker"
    );
}
...
fn test_table_get_user_key_mismatch() {
    ...
    assert!(
        result.is_none(),
        "Should return None when user_key doesn't match"
    );
}
...
fn test_range_at_exact_block_boundary() {
    ...
    let collected = collect_all(&mut iter).unwrap();
    let results: Vec<String> = collected
        .iter()
...
fn test_excluded_bound_skips_all_versions() {
    ...
    assert!(
        !results.iter().any(|k| k == "key_002"),
        "key_002 should not appear in results"
    );
}
...
fn test_reverse_iteration_with_excluded_upper_bound() {
    ...
    assert!(
        !results.iter().any(|k| k == "key_003"),
        "key_003 should not appear"
    );
    assert!(
        !results.iter().any(|k| k == "key_004"),
        "key_004 should not appear"
    );
...
fn test_empty_range_returns_nothing() {
    ...
    assert_eq!(
        results.len(),
        0,
        "Range for non-existent key should be empty"
    );
}
...
fn test_reverse_iteration_empty_range() {
    ...
    assert!(
        ...
        "Empty range should be invalid on seek_last()"
    );
}
...
fn test_table_properties_persistence() {
    ...
    assert_eq!(
        props.num_deletions, expected_deletions,
        "Number of deletions should match"
    );
    assert_eq!(
        props.tombstone_count, expected_tombstones,
        "Tombstone count should match"
    );
    assert!(
        props.num_data_blocks > 0,
    ...
    assert!(
        ...
        "Should have at least one index partition"
    );
    assert!(
        props.top_level_index_size > 0,
        "Top-level index size should be tracked"
    );
    ...
    assert_eq!(
        props.seqnos.0, 1000,
        "Smallest sequence number should be 1000"
    );
    assert_eq!(
        props.seqnos.1, 1049,
        "Largest sequence number should be 1049"
    );
    assert_eq!(
        meta.smallest_seq_num,
        Some(1000),
        "Metadata smallest seq num should match"
    );
    assert_eq!(
        meta.largest_seq_num,
        Some(1049),
        "Metadata largest seq num should match"
    );

    // Compression
    assert_eq!(
        props.compression, expected_compression,
        "Compression should match table settings"
    );
    ...
    assert_eq!(
        props.oldest_key_time,
        Some(0),
        "Oldest key time should be 0"
    );
    assert_eq!(
        props.newest_key_time,
        Some(0),
        "Newest key time should be 0"
    );

    // Range deletion metrics
    assert_eq!(
        props.num_range_deletions, 5,
        "Should have 5 range deletions"
    );
    ...
    assert!(
        meta.smallest_point.is_some(),
        "Should have smallest point key"
    );
    assert!(
        meta.largest_point.is_some(),
        "Should have largest point key"
    );
    if let Some(smallest) = &meta.smallest_point {
        assert_eq!(
            &smallest.user_key, b"key_000",
            "Smallest key should be key_000"
        );
        assert_eq!(smallest.seq_num(), 1000, "Smallest seq num should be 1000");
    }
    if let Some(largest) = &meta.largest_point {
        assert_eq!(
            &largest.user_key, b"key_049",
            "Largest key should be key_049"
        );
...
fn test_table_get_nonexistent_keys() {
    ...
    for key_str in &nonexistent_keys {
        ...
        if result.is_some() {
            ...
            assert!(
                found_str.starts_with("key_"),
                "Found key {found_str} should be valid"
            );
        }
    }
}
...
fn test_table_get_with_compression() {
    ...
    let compression_types = vec![
        ...
        crate::CompressionType::SnappyCompression,
    ];
    ...
    for compression in compression_types {
        ...
        for i in 0..20 {
            ...
            assert!(
                result.is_some(),
                "Should find key {key} with compression {compression:?}"
            );
...
fn test_table_iterator_reverse_scan() {
    ...
    for (i, (key, value)) in collected.iter().enumerate() {
        ...
        assert_eq!(
            *key,
            format!("key_{expected_idx:03}"),
            "Reverse key order mismatch"
        );
        assert_eq!(
            *value,
            format!("value_{expected_idx:03}"),
            "Reverse value mismatch"
        );
    }
}
...
fn test_table_iterator_seek_and_scan() {
    ...
    assert_eq!(
        std::str::from_utf8(last_key_bytes).unwrap(),
        "key_029",
...
fn test_table_iterator_across_partitions() {
    ...
    assert!(
        partitioned_index.blocks.len() >= 2,
    ...
    assert_eq!(
        count_backward, 100,
        "Should iterate all 100 keys backwards across partitions"
    );
}
...
fn test_backward_iter_multiple_versions_same_user_key() {
    ...
    assert_eq!(
        results[1].1, 200,
        "Second backward result should be seq=200"
    );
}
...
fn test_metadata_roundtrip_with_edge_values() {
    ...
    // Test case 1: seq_num = 0
    {
        ...
        assert_eq!(
            decoded.smallest_seq_num,
            Some(0),
            "Roundtrip should preserve seq_num=0"
        );
