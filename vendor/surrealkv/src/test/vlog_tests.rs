...
async fn test_vlog_rotation_uses_projected_append_size() {
	...
	assert!(file1_len <= max_file_size, "File 1 should remain within configured max size");
	assert!(
...
async fn test_vlog_single_oversized_entry_stays_atomic() {
	...
	assert!(file1_len > max_file_size, "Single oversized entry is allowed to exceed max file size");
