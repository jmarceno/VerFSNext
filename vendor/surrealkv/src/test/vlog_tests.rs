...
async fn test_value_location_from_encoded_value_vlog() {
    ...
    let decoded_location = ValueLocation::decode(&encoded).unwrap();
    let resolved = decoded_location
        .clone()
        .resolve_value(Some(&Arc::new(vlog)))
        .unwrap();
    assert_eq!(&*resolved, value);
...
fn test_vlog_file_header_encoding() {
    ...
    let header = VLogFileHeader::new(123, opts.vlog_max_file_size, CompressionType::Lz4 as u8);
...
fn test_vlog_file_header_invalid_magic() {
    let opts = Options::default();
    let mut header = VLogFileHeader::new(123, opts.vlog_max_file_size, CompressionType::Lz4 as u8);
...
async fn test_vlog_restart_continues_last_file() {
    ...
    // file)
    {
        ...
        assert!(
            file_size < vlog_max_file_size,
            "File should not be at max capacity"
        );
...
async fn test_vlog_restart_with_multiple_files() {
    ...
    // Phase 2: Restart VLog and verify it picks up the correct active writer
    {
        ...
        let has_new_file = more_pointers
            .iter()
            .any(|(p, _)| p.file_id > highest_file_id);
        if has_new_file {
        ...
        for (pointer, expected_value) in &more_pointers {
            ...
            assert_eq!(
                *retrieved, *expected_value,
                "Should be able to read bulk-added data"
            );
        }
...
#[test(tokio::test)]
async fn test_vlog_rotation_uses_projected_append_size() {
    let temp_dir = TempDir::new().unwrap();
    ...
    std::fs::create_dir_all(opts.vlog_dir()).unwrap();
    let vlog = VLog::new(Arc::clone(&opts)).unwrap();

    // First write leaves room in file 1.
    let pointer1 = vlog.append(b"small", &vec![1u8; 200]).unwrap();
    assert_eq!(pointer1.file_id, 1);

    // This entry itself fits in a file, but would overflow if appended to file 1.
    let pointer2 = vlog.append(b"big_entry", &vec![2u8; 760]).unwrap();
    vlog.sync().unwrap();
    assert_eq!(pointer2.file_id, 2, "Append should rotate before overflow");

    let file1_path = vlog.vlog_file_path(1);
    let file2_path = vlog.vlog_file_path(2);
    let file1_len = std::fs::metadata(&file1_path).unwrap().len();
    let file2_len = std::fs::metadata(&file2_path).unwrap().len();

    assert!(
        file1_len <= max_file_size,
        "File 1 should remain within configured max size"
    );
    assert!(
        file2_len <= max_file_size,
        "File 2 should remain within configured max size for this test case"
    );

    let got1 = vlog.get(&pointer1).unwrap();
    let got2 = vlog.get(&pointer2).unwrap();
    assert_eq!(*got1, vec![1u8; 200]);
    assert_eq!(*got2, vec![2u8; 760]);
}

#[test(tokio::test)]
async fn test_vlog_single_oversized_entry_stays_atomic() {
    let temp_dir = TempDir::new().unwrap();
    let max_file_size = 1024;
    let opts = Arc::new(Options {
        path: temp_dir.path().to_path_buf(),
    ...
    // Oversized single entry cannot be split across files.
    let oversized = vec![9u8; 1400];
    let pointer1 = vlog.append(b"oversized", &oversized).unwrap();
    vlog.sync().unwrap();
    assert_eq!(pointer1.file_id, 1);

    let file1_len = std::fs::metadata(vlog.vlog_file_path(1)).unwrap().len();
    assert!(
        file1_len > max_file_size,
        "Single oversized entry is allowed to exceed max file size"
    );

    // Next append should rotate away from the oversized active file.
    let pointer2 = vlog.append(b"next", &vec![3u8; 64]).unwrap();
    vlog.sync().unwrap();
    assert_eq!(pointer2.file_id, 2);

    let got1 = vlog.get(&pointer1).unwrap();
    let got2 = vlog.get(&pointer2).unwrap();
    assert_eq!(*got1, oversized);
    assert_eq!(*got2, vec![3u8; 64]);
}
...
async fn test_vlog_writer_reopen_append_only_behavior() {
    ...
    // corruption/overwriting
    {
            ...
            phase2_pointers.iter().zip(phase2_data.iter()).enumerate()
        {
            ...
            assert_eq!(
                *retrieved, *expected_value,
                "Phase 2 entry {} should be readable",
                i
            );
        }
    ...
    let all_offsets: Vec<u64> = phase1_pointers
        .iter()
        .chain(phase2_pointers.iter())
        .map(|p| p.offset)
        .collect();
