...
impl std::fmt::Debug for SnapshotTracker {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SnapshotTracker")
            .field("snapshots", &self.get_all_snapshots())
            .finish()
    }
}
...
impl Snapshot {
    fn encode_i64_as_inline_value_location(value: i64) -> Value {
        ValueLocation::with_inline_value(value.to_le_bytes().to_vec()).encode()
    }

    fn decode_counter_from_encoded_value(&self, encoded_value: &[u8]) -> Result<i64> {
        let resolved = self.core.resolve_value(encoded_value)?;
        if resolved.len() != 8 {
            return Err(Error::InvalidArgument(
                "Counter value must be fixed-width little-endian i64 (8 bytes)".to_string(),
            ));
        }

        let mut bytes = [0u8; 8];
        bytes.copy_from_slice(&resolved);
        Ok(i64::from_le_bytes(bytes))
    }

    fn key_in_half_open_range(&self, key: &[u8], start: &[u8], end: &[u8]) -> bool {
        self.core.opts.comparator.compare(key, start) != Ordering::Less
            && self.core.opts.comparator.compare(key, end) == Ordering::Less
    }

    fn maybe_update_covering_range_delete_seq(
        &self,
        target_key: &[u8],
        max_seq: u64,
        key_ref: &InternalKeyRef<'_>,
        encoded_value: &[u8],
        best_seq: &mut Option<u64>,
    ) -> Result<()> {
        if key_ref.kind() != InternalKeyKind::RangeDelete || key_ref.seq_num() > max_seq {
            return Ok(());
        }

        let end = self.core.resolve_value(encoded_value)?;
        if !self.key_in_half_open_range(target_key, key_ref.user_key(), &end) {
            return Ok(());
        }

        let seq = key_ref.seq_num();
        if best_seq.is_none_or(|current| seq > current) {
            *best_seq = Some(seq);
        }
    ...
    fn find_latest_covering_range_delete_seq(
        &self,
        key: &[u8],
        max_seq: u64,
    ) -> Result<Option<u64>> {
...

        {
            ...
            while iter.valid() {
                ...
                self.maybe_update_covering_range_delete_seq(
                    key,
                    max_seq,
                    &key_ref,
                    iter.value_encoded()?,
        ...
        {
            let immutables = self.core.immutable_memtables.read()?;
            for entry in immutables.iter().rev() {
                let mut iter = entry.memtable.iter();
                iter.seek_first()?;
                while iter.valid() {
                    let key_ref = iter.key();
                    self.maybe_update_covering_range_delete_seq(
        ...
        {
            let levels = self.core.level_manifest.read()?;
    ...
    fn may_have_any_range_deletes(&self) -> Result<bool> {
        {
            let active = self.core.active_memtable.read()?;
            if active.may_have_range_deletes() {
                return Ok(true);
            }
        }

        {
            ...
            if immutables
                .iter()
                .any(|entry| entry.memtable.may_have_range_deletes())
            {
                return Ok(true);
            }
        }
        ...
        for level in &levels.levels {
            if level
                .tables
                .iter()
                .any(|table| table.meta.properties.num_range_deletions > 0)
            {
                return Ok(true);
            }
        }
    ...
    fn get_latest_point_entry_at_or_below(
        &self,
        key: &[u8],
        max_seq: u64,
        ...
        let mut maybe_update = |candidate: (InternalKey, Value)| {
            if candidate.0.seq_num() > max_seq {
                return;
            }
            if best
                .as_ref()
                .is_none_or(|current| candidate.0.seq_num() > current.0.seq_num())
            {
                best = Some(candidate);
            }
        };
        ...
        {
            let active = self.core.active_memtable.read()?;
            if let Some(item) = active.get(key, Some(max_seq)) {
                maybe_update(item);
            }
        }

        {
            let immutables = self.core.immutable_memtables.read()?;
            for entry in immutables.iter().rev() {
                if let Some(item) = entry.memtable.get(key, Some(max_seq)) {
                    maybe_update(item);
                }
            }
        }

        let search_key = InternalKey::new(key.to_vec(), max_seq, InternalKeyKind::Set, 0);
        {
            let level_manifest = self.core.level_manifest.read()?;
            let tiered_levels = self.core.opts.compaction_tiered_levels as usize;
            for (level_idx, level) in (&level_manifest.levels).into_iter().enumerate() {
                if level_idx < tiered_levels {
                    for table in &level.tables {
                        if !table.is_key_in_key_range(&search_key) {
                            continue;
                        }

                        if let Some(item) = table.get(&search_key)? {
                            maybe_update(item);
                        }
                    }
                } else {
    ...
    pub(crate) fn get(&self, key: &[u8]) -> crate::Result<Option<(Value, u64)>> {
        let range_delete_seq = if self.may_have_any_range_deletes()? {
            self.find_latest_covering_range_delete_seq(key, self.seq_num)?
        } else {
            None
        };
        let mut max_seq = self.seq_num;
        ...
        loop {
            ...
            match internal_key.kind() {
                InternalKeyKind::Set | InternalKeyKind::Replace => {
                    if !has_merge {
                        return Ok(Some((encoded_value, internal_key.seq_num())));
                    }

                    let base = self.decode_counter_from_encoded_value(&encoded_value)? as i128;
                    let total = accumulated_delta.checked_add(base).ok_or_else(|| {
                        Error::InvalidArgument(
                            "Counter merge overflow while resolving value".to_string(),
                        )
                    })?;
                    ...
                    return Ok(Some((
                        Self::encode_i64_as_inline_value_location(total_i64),
                ...
                | InternalKeyKind::RangeDelete => {
                    ...
                    let total_i64 = i64::try_from(accumulated_delta).map_err(|_| {
                        Error::InvalidArgument(
                            "Counter merge overflow while resolving value".to_string(),
                        )
                    })?;
                ...
                InternalKeyKind::Merge => {
                    let delta = self.decode_counter_from_encoded_value(&encoded_value)? as i128;
                    accumulated_delta = accumulated_delta.checked_add(delta).ok_or_else(|| {
                        Error::InvalidArgument(
                            "Counter merge overflow while resolving value".to_string(),
                        )
                    })?;
            ...
            if internal_key.seq_num() == 0 {
                break;
            }
            max_seq = internal_key.seq_num() - 1;
        }

        if has_merge {
            let total_i64 = i64::try_from(accumulated_delta).map_err(|_| {
                Error::InvalidArgument("Counter merge overflow while resolving value".to_string())
            })?;
            return Ok(Some((
                Self::encode_i64_as_inline_value_location(total_i64),
                self.seq_num,
    ...
    pub(crate) fn btree_history_iter(&self) -> Result<BPlusTreeIteratorWithGuard<'_>> {
        if !self.core.opts.enable_versioned_index {
            return Err(Error::InvalidArgument(
                "B+tree versioned index not enabled".to_string(),
            ));
        }
    ...
    pub(crate) fn history_iter(
        ...
        if self.core.opts.enable_versioned_index {
            ...
            Ok(HistoryIterator::new_lsm(
                ...
                ts_range,
                limit,
                self.core.opts.compaction_tiered_levels,
            ))
        }
    }
...
impl<'a> KMergeIterator<'a> {
    ...
    pub(crate) fn new_from(
        ...
        let cmp: Arc<dyn Comparator> = Arc::new(InternalKeyComparator::new(Arc::new(
            BytewiseComparator::default(),
        )));
        Self::new_with_comparator(iter_state, internal_range, cmp, None, tiered_levels)
    }
    ...
    pub(crate) fn new_for_history(
        ...
        let cmp: Arc<dyn Comparator> = Arc::new(TimestampComparator::new(Arc::new(
            BytewiseComparator::default(),
        )));
        Self::new_with_comparator(iter_state, internal_range, cmp, ts_range, tiered_levels)
    }
    ...
    fn new_with_comparator(
        ...
        let tiered_levels = tiered_levels as usize;
        for (level_idx, level) in (&state_ref.levels).into_iter().enumerate() {
            ...
            if level_idx < tiered_levels {
                // Tiered levels: tables may overlap, so check each table.
                for table in &level.tables {
                ...
                // Leveled levels: non-overlapping ranges allow binary-search slicing.
                let start_idx = level.find_first_overlapping_table(&query_range);
                let end_idx = level.find_last_overlapping_table(&query_range);
...
pub(crate) struct SnapshotIterator<'a> {
    ...
    /// Resolved value for current forward key (merge/range-delete aware).
    forward_resolved_value: Vec<u8>,
    has_forward_resolved_value: bool,
...
impl SnapshotIterator<'_> {
    ...
    fn new_from(core: Arc<Core>, seq_num: u64, range: InternalKeyRange) -> Result<Self> {
        ...
        let merge_iter =
            KMergeIterator::new_from(iter_state, range, core.opts.compaction_tiered_levels);
        ...
        Ok(Self {
            ...
            direction: MergeDirection::Forward,
            forward_resolved_value: Vec::new(),
            has_forward_resolved_value: false,
            initialized: false,
        })
    }
    ...
    fn skip_to_valid_forward(&mut self) -> Result<bool> {
        while self.merge_iter.valid() {
            ...
            let snapshot = Snapshot {
                ...
                seq_num: self.snapshot_seq_num,
            };

            match snapshot.get(user_key)? {
                Some((resolved, _)) => {
                    self.forward_resolved_value.clear();
                    self.forward_resolved_value.extend_from_slice(&resolved);
                    self.has_forward_resolved_value = true;
                    return Ok(true);
                }
                None => {
                    self.merge_iter.next()?;
    ...
    fn find_latest_visible_backward(&mut self) -> Result<bool> {
        ...
        loop {
            ...
            if user_key != current_user_key.as_slice() {
                ...
                self.buffered_back_value
                    .extend_from_slice(self.merge_iter.value_encoded()?);
...
impl LSMIterator for SnapshotIterator<'_> {
    fn seek(&mut self, target: &[u8]) -> Result<bool> {
        self.direction = MergeDirection::Forward;
        self.last_key_fwd.clear();
        self.has_forward_resolved_value = false;
        self.has_buffered_back = false;
        self.has_current_back = false;
    ...
    fn value_encoded(&self) -> Result<&[u8]> {
        ...
        } else if self.has_forward_resolved_value {
            Ok(&self.forward_resolved_value)
        } else {
...
impl<'a> HistoryIterator<'a> {
    pub(crate) fn new_lsm(
        ...
        let inner = if ts_range.is_some() {
            HistoryIteratorInner::Lsm(KMergeIterator::new_for_history(
                iter_state,
                range,
                ts_range,
                tiered_levels,
            ...
            HistoryIteratorInner::Lsm(KMergeIterator::new_from(iter_state, range, tiered_levels))
        };
