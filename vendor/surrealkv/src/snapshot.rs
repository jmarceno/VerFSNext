...
impl Snapshot {
	...
	fn find_latest_covering_range_delete_seq(&self, key: &[u8], max_seq: u64) -> Result<Option<u64>> {
	...
	fn may_have_any_range_deletes(&self) -> Result<bool> {
		...
		{
			let immutables = self.core.immutable_memtables.read()?;
			if immutables.iter().any(|entry| entry.memtable.may_have_range_deletes()) {
				return Ok(true);
			}
		}
	...
	pub(crate) fn get(&self, key: &[u8]) -> crate::Result<Option<(Value, u64)>> {
		...
		if has_merge {
			...
			return Ok(Some((Self::encode_i64_as_inline_value_location(total_i64), self.seq_num)));
		}
