...
pub enum BPlusTreeError {
    ...
    InvalidRootNode(u64),
    InconsistentFreePageCount { claimed: u32, actual: u32 },
    Corruption(String),
}
...
impl std::fmt::Display for BPlusTreeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            BPlusTreeError::InconsistentFreePageCount { claimed, actual } => {
                write!(
                    f,
...
impl LeafNode {
    ...
    fn delete(&mut self, key: &[u8], compare: &dyn Comparator) -> Option<(usize, Bytes, u64)> {
        let idx = self
            .keys
            .binary_search_by(|k| compare.compare(k, key))
            .ok()?;
...
fn leaf_cell_size(key: &[u8], value: &[u8]) -> usize {
    ...
    KEY_SIZE_PREFIX + VALUE_SIZE_PREFIX + bytes_on_page + if needs_overflow { 8 } else { 0 }
}
...
fn internal_entry_size(key: &[u8]) -> usize {
    ...
    KEY_SIZE_PREFIX + bytes_on_page + if needs_overflow { 8 } else { 0 }
}
...
impl<F: VfsFile> BPlusTree<F> {
    ...
    fn split_leaf(
        ...
        #[cfg(debug_assertions)]
        {
            ...
            assert!(
                right_size <= PAGE_SIZE,
                "Right leaf {} > PAGE_SIZE {}",
                right_size,
                PAGE_SIZE
            );
...
impl<'a, F: VfsFile> RangeScanIterator<'a, F> {
    pub(crate) fn new(
        ...
        let current_idx = if start_key.is_empty() {
            ...
            match start {
                Bound::Included(key) => leaf
                    .keys
                    .partition_point(|k| tree.compare.compare(k, key) == Ordering::Less),
                Bound::Excluded(key) => {
                    // Find first key > start_key
                    // partition_point returns first index where compare(k, key) ==
                    // Ordering::Greater
                    leaf.keys
                        .partition_point(|k| tree.compare.compare(k, key) != Ordering::Greater)
                }
                Bound::Unbounded => 0,
            }
        };
...
mod tests {
    ...
    fn generate_random_values(n: usize, value_size: usize) -> Vec<Vec<u8>> {
        ...
        (0..n)
            .map(|_| (0..value_size).map(|_| rng.random::<u8>()).collect())
            .collect()
    }
    ...
    fn test_single_insert_search() {
        ...
        tree.insert(b"key1", b"value1").unwrap();
        assert_eq!(
            tree.get(b"key1").unwrap().as_deref(),
    ...
    fn test_update() {
        ...
        assert_eq!(
            tree.get(b"key2").unwrap().as_deref(),
            Some(b"updated_value2".as_ref())
        );
        assert_eq!(
            tree.get(b"key3").unwrap().as_deref(),
    ...
    fn test_drop_behavior() {
        ...
        // Verify data post drop
        {
            ...
            assert_eq!(
                tree.get(b"test").unwrap().as_deref(),
                Some(b"value".as_ref())
            );
        }
    }
    ...
    fn test_range_basic() {
        ...
        // Test range 3-7
        let results = tree
            .range(serialize_u32(3).as_slice()..=serialize_u32(7).as_slice())
            .unwrap();
        let expected: Vec<_> = (3..=7).map(|i| (i, i * 10)).collect();
        assert_eq!(
            results
                .into_iter()
                .map(|res| deserialize_pair(res.unwrap()))
                .collect::<Vec<_>>(),
            expected
        );
    }
    ...
    fn test_range_spanning_leaves() {
        ...
        // Test range 5-15
        let results = tree
            .range(serialize_u32(5).as_slice()..=serialize_u32(15).as_slice())
            .unwrap();
    ...
    fn test_invalid_range() {
        ...
        let results: Vec<_> = tree
            .range(serialize_u32(3).as_slice()..serialize_u32(1).as_slice())
            .unwrap()
            .collect();
    ...
    fn test_missing_boundaries() {
        ...
        let results = tree
            .range(serialize_u32(2).as_slice()..=serialize_u32(9).as_slice())
            .unwrap();
    ...
    fn test_insert_with_multiple_key_sizes() {
        ...
        for key_size in &key_sizes {
            ...
            for i in 0..*key_size {
                ...
                tree.insert(unique_key.clone(), fixed_value.clone())
                    .unwrap();
            }
    ...
    fn test_internal_iterator_two_elements() {
        ...
        tree.insert(make_internal_key(b"first", 1), b"val1")
            .unwrap();
        tree.insert(make_internal_key(b"second", 2), b"val2")
            .unwrap();
