...
impl BlockIterator {
    ...
    pub(crate) fn new(comparator: Arc<dyn Comparator>, block: BlockData) -> Result<Self> {
        ...
        let restart_offset = block
            .len()
            .checked_sub(4 * (num_restarts + 1))
            .ok_or_else(|| {
    ...
    fn decode_entry_lengths(&self, offset: usize) -> Result<(usize, usize, usize, usize)> {
        ...
        let (shared_prefix_length, shared_prefix_length_size) =
            usize::decode_var(&self.block[offset..])
                .ok_or_else(|| Error::from(SSTableError::FailedToDecodeSharedPrefix { offset }))?;
        ...
        let (non_shared_key_length, non_shared_key_length_size) =
            usize::decode_var(&self.block[offset + i..]).ok_or_else(|| {
                Error::from(SSTableError::FailedToDecodeNonSharedKeyLength { offset: offset + i })
            })?;
        i += non_shared_key_length_size;

        let (value_size, value_size_size) = usize::decode_var(&self.block[offset + i..])
            .ok_or_else(|| {
                Error::from(SSTableError::FailedToDecodeValueSize { offset: offset + i })
            })?;
        i += value_size_size;
    ...
    fn seek_next_entry(&mut self) -> Result<()> {
        ...
        let key_end = self
            .offset
            .checked_add(i)
            .and_then(|sum| sum.checked_add(non_shared_key))
            .ok_or_else(|| {
        ...
        self.current_key
            .extend_from_slice(&self.block[self.offset + i..key_end]);
    ...
    pub(crate) fn prev_internal(&mut self) -> Result<bool> {
        ...
        loop {
            match self.seek_next_entry() {
                ...
                Err(e) => {
                    if let Error::SSTable(SSTableError::OffsetExceedsRestartOffset { .. }) = e {
                        // Expected EOF
                        return Ok(false);
                    }
                    return Err(e);
