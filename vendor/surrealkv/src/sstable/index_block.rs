...
impl Index {
    ...
    pub(crate) fn find_block_handle_by_key(
        ...
        if self.blocks.is_empty() {
            let err = Error::from(SSTableError::EmptyCorruptPartitionedIndex { table_id: self.id });
            log::error!("[INDEX] {}", err);
            return Err(err);
        }
...
impl<'a> IndexIterator<'a> {
    ...
    pub(crate) fn seek_to_last(&mut self) -> Result<()> {
        ...
        let block = self
            .index
            .load_block(&self.index.blocks[self.partition_index])?;
        let mut iter = block.iter()?;
        iter.seek_to_last()?;
        self.partition_iter = Some(iter);
