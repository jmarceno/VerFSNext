...
impl Transaction {
    fn encode_counter_delta(op: MergeOp) -> Value {
        let delta = match op {
    ...
    pub(crate) fn new(core: Arc<Core>, opts: TransactionOptions) -> Result<Self> {
        let TransactionOptions { mode, durability } = opts;
        ...
        V: IntoBytes,
    {
        self.set_with_options(
            key,
            value,
            &WriteOptions::default().with_timestamp(Some(timestamp)),
        )
    }
    ...
    /// Atomically appends a merge delta for a counter key.
    ///
    /// Counter payloads are encoded as fixed-width little-endian i64.
    pub fn merge<K>(&mut self, key: K, op: MergeOp, _missing: MergeMissingBehavior) -> Result<()>
    where
        K: IntoBytes,
    {
        self.merge_with_options(key, op, _missing, &WriteOptions::default())
    }

    /// Atomically appends a merge delta for a counter key with custom write options.
    pub fn merge_with_options<K>(
        &mut self,
        key: K,
        op: MergeOp,
        _missing: MergeMissingBehavior,
        ...
        K: IntoBytes,
    {
        let write_seqno = self.next_write_seqno();
        let ts = options.timestamp.unwrap_or(Entry::COMMIT_TIME);
        let delta = Self::encode_counter_delta(op);
    ...
    /// Inserts a key only when it does not exist in the transaction view.
    ///
    /// This method guarantees read-your-own-writes correctness for the transaction.
    /// Tree-level `put_if_not_exists` provides single-winner admission across concurrent commits.
    pub fn put_if_not_exists<K, V>(&mut self, key: K, value: V) -> Result<PutIfAbsentResult>
    where
        K: IntoBytes,
        V: IntoBytes,
    {
        if self.get(key.as_slice())?.is_some() {
    ...
    where
        K: IntoBytes,
    {
        self.delete_range_with_options(start, end, &WriteOptions::default())
    }

    /// Deletes keys in half-open range [start, end) by writing a range tombstone,
    /// with custom write options.
    pub fn delete_range_with_options<K>(
        &mut self,
        start: K,
        end: K,
        options: &WriteOptions,
        ...
        K: IntoBytes,
    {
        ...
        if !self.core.opts.enable_versioning {
            return Err(Error::InvalidArgument(
                "Versioned queries not enabled".to_string(),
            ));
        }
        ...
        K: IntoBytes,
    {
        ...
        for (key, entry_list) in self
            .write_set
            .range(start.as_slice().to_vec()..end.as_slice().to_vec())
        {
    ...
    pub async fn commit(&mut self) -> Result<()> {
        ...
        let mut latest_writes: Vec<Entry> = std::mem::take(&mut self.write_set)
            .into_values()
            .flatten()
            .collect();
...
impl<'a> TransactionRangeIterator<'a> {
    ...
    fn populate_ws_encoded_key(&mut self) {
        ...
        self.ws_encoded_key_buf
            .extend_from_slice(&trailer.to_be_bytes());
        self.ws_encoded_key_buf
            .extend_from_slice(&entry.timestamp.to_be_bytes());
    }
