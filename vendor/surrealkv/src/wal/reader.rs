...
mod tests {
    ...
    fn reader_record_spanning_blocks() {
        ...
        wal.append(&[1, 2, 3, 4])
            .expect("should append small record");
    ...
    fn writer_block_boundary_exact_fit() {
        ...
        writer
            .add_record(&data)
            .expect("should write exact fit record");

        // Write another small record (should go to next block)
        writer
            .add_record(b"next")
            .expect("should write next record");
        ...
        assert_eq!(
            offset1, BLOCK_SIZE as u64,
            "First record fills entire block"
        );
    ...
    fn many_blocks() {
        ...
        for i in 0..num_records {
            ...
            let (data, _) = reader
                .read()
                .unwrap_or_else(|_| panic!("should read record {}", i));
    ...
    fn truncated_trailing_record() {
        ...
        assert!(
            reader.read().is_err(),
            "Should return EOF on truncated record"
        );
    }
    ...
    fn missing_last() {
        ...
        assert!(
            result.is_err(),
            "Should return error for missing Last fragment"
        );
    }
    ...
    fn original_crud_bench_bug_reproduction() {
        ...
        loop {
            ...
            for i in 0..records_per_session {
                let data: Vec<u8> = (0..record_size)
                    .map(|j| ((total_records + i + j) & 0xFF) as u8)
                    .collect();
                wal.append(&data).expect("should append");
            }
            wal.close().expect("should close");
    ...
    fn mixed_fragmented_and_small_records() {
        ...
        for session in 0..5 {
            ...
            let large_data: Vec<u8> = (0..large_size)
                .map(|j| ((session * 1000 + j) & 0xFF) as u8)
                .collect();
            ...
            for i in 0..10 {
                ...
                let small_data: Vec<u8> = (0..small_size)
                    .map(|j| ((session * 100 + i + j) & 0xFF) as u8)
                    .collect();
    ...
    fn exact_block_alignment_stress() {
        ...
        for session in 0..5 {
            ...
            let data: Vec<u8> = (0..exact_fit_size)
                .map(|j| ((session + j) & 0xFF) as u8)
                .collect();
            expected_records.push(data.clone());
            wal.append(&data).expect("should append exact fit");

            // Write a small record after (tests resuming from block boundary)
            let small_data: Vec<u8> = (0..100)
                .map(|j| ((session * 10 + j) & 0xFF) as u8)
                .collect();
            expected_records.push(small_data.clone());
            wal.append(&small_data).expect("should append small");
