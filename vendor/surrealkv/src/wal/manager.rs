...
mod tests {
    ...
    fn test_cleanup_stale_repair_files() {
        ...
        fs::write(
            wal_dir.join("00000000000000000000.wal.repair"),
            b"stale repair 1",
        )
        .unwrap();
        fs::write(
            wal_dir.join("00000000000000000001.wal.repair"),
            b"stale repair 2",
        )
        .unwrap();
    ...
    fn test_wal_append_to_existing() {
        ...
        assert!(temp_dir.path().join("00000000000000000000.wal").exists());
        let size_after_first = fs::metadata(temp_dir.path().join("00000000000000000000.wal"))
            .unwrap()
            .len();
        ...
        let size_after_second = fs::metadata(temp_dir.path().join("00000000000000000000.wal"))
            .unwrap()
            .len();
        assert!(
    ...
    fn test_wal_block_offset_across_sessions() {
        ...
        for (i, record) in test_records.iter().enumerate() {
            ...
            let size = fs::metadata(temp_dir.path().join("00000000000000000000.wal"))
                .unwrap()
                .len();
        ...
        assert_eq!(
            files.len(),
            1,
            "Should have only one WAL file after all sessions"
        );
    ...
    fn test_open_with_min_log_number_uses_highest_segment() {
        ...
        // disk), not 1
        {
            ...
            log::info!(
                "After open_with_min_log_number(1): active_log_number = {}",
                active
            );
