...
impl ImmutableMemtables {
    ...
    pub(crate) fn remove(&mut self, id_to_remove: u64) {
        if let Ok(index) = self
            .0
            .binary_search_by_key(&id_to_remove, |entry| entry.table_id)
        {
            self.0.remove(index);
        }
    }
...
impl MemTable {
    pub(crate) fn new(arena_capacity: usize, earliest_seq: u64) -> Self {
        ...
        MemTable {
            ...
            has_range_deletes: AtomicBool::new(false),
            earliest_seq,
            wal_number: AtomicU64::new(0),
        }
    }

    pub(crate) fn may_have_range_deletes(&self) -> bool {
        self.has_range_deletes.load(Ordering::Acquire)
    }
    ...
    fn apply_batch_to_memtable(&self, batch: &Batch) -> Result<u64> {
        ...
        for (_i, entry, current_seq_num, timestamp) in batch.entries_with_seq_nums()? {
            if entry.kind == crate::InternalKeyKind::RangeDelete {
                self.has_range_deletes.store(true, Ordering::Release);
            }
    ...
    fn insert_into_memtable(&self, key: &InternalKey, value: &Value) -> Result<()> {
        ...
        match self
            .skiplist
            .add(&key.user_key, trailer, key.timestamp, value)
        {
            Ok(()) => Ok(()),
