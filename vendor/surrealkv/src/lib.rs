...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub struct VLogGcMetrics {
    pub relocated_entries: u64,
...
pub struct Options {
    ...
    /// Per-level bloom filter bits-per-key for Monkey-style allocation.
    ///
    /// Empty means "disabled" (use `filter_policy` as-is).
    /// Index corresponds to level number. Higher levels beyond vector length
    /// use the last configured value.
    ///
    /// This keeps filter format and policy name stable while allowing
    /// level-aware false-positive tuning during SST creation.
    pub bloom_bits_per_key_per_level: Vec<usize>,
    ...
    pub vlog_compression: CompressionType,
    ...
    /// Controls background GC behavior.
    /// Level compaction remains enabled independently.
    ...
    /// Number of top levels that use tiered compaction behavior.
    /// Levels in `[0, compaction_tiered_levels)` are treated as tiered.
    /// Remaining levels use leveled compaction.
    /// Default: 1 (only L0 is tiered-style)
    pub compaction_tiered_levels: u8,
}
...
impl Default for Options {
    fn default() -> Self {
        ...
        Self {
            ...
            bloom_bits_per_key_per_level: Vec::new(),
            ...
            vlog_compression: CompressionType::Lz4,
            enable_vlog: false,
...
impl Options {
    ...
    /// Sets Monkey-style per-level bloom filter bits-per-key.
    ///
    /// Empty vector disables level-aware bloom tuning.
    /// Index maps to level number (`0 = L0`), with last value reused for deeper
    /// levels.
    pub fn with_bloom_bits_per_key_per_level(mut self, bits: Vec<usize>) -> Self {
        self.bloom_bits_per_key_per_level = bits;
        self
    }
    ...
    pub const fn with_enable_background_gc(mut self, value: bool) -> Self {
        self.enable_background_gc = value;
        self
    }
    ...
    /// Sets the number of top levels that use tiered compaction behavior.
    ///
    /// Value must be in `[1, level_count]`.
    pub const fn with_compaction_tiered_levels(mut self, value: u8) -> Self {
        self.compaction_tiered_levels = value;
        self
    }
    ...
    pub const fn with_vlog_compression(mut self, value: CompressionType) -> Self {
        self.vlog_compression = value;
        self
    }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.compaction_tiered_levels == 0 || self.compaction_tiered_levels > self.level_count {
            return Err(Error::InvalidArgument(format!(
                "compaction_tiered_levels must be in [1, level_count], got {} with level_count={}",
                self.compaction_tiered_levels, self.level_count
            )));
        }
