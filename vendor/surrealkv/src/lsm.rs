		...
		for (idx, entry, current_seq_num, timestamp) in batch.entries_with_seq_nums()? {
			...
			loop {
				...
				match result {
					Ok(()) => break,
					Err(Error::ArenaFull) => {
						log::debug!("apply: arena full during single-entry apply, rotating memtable");
...
impl Tree {
		...
		K: IntoBytes,
	{
		...
		let mut batch = Batch::new(0);
		let ts = self.core.inner.opts.clock.now();
		batch.add_record(InternalKeyKind::RangeDelete, start_bytes, Some(end_bytes), ts)?;
