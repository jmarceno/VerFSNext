...
use crate::transaction::{
    MergeMissingBehavior, MergeOp, Mode, PutIfAbsentResult, Transaction, TransactionOptions,
};
...
pub trait CompactionOperations: Send + Sync {
    ...
    /// tombstones.
    fn compact(&self, strategy: Arc<dyn CompactionStrategy>) -> Result<bool>;
...
impl CoreInner {
    ...
    pub(crate) fn new(opts: Arc<Options>) -> Result<Self> {
        ...
        let versioned_index = if opts.enable_versioned_index {
            ...
            let comparator = Arc::new(TimestampComparator::new(Arc::new(
    ...
    fn flush_and_update_manifest(
        ...
        let table = memtable
            .flush(table_id, Arc::clone(&self.opts))
            .map_err(|e| {
                Error::Other(format!(
    ...
    pub(crate) fn rotate_memtable(&self) -> Result<()> {
        ...
        log::debug!(
            "rotate_memtable: rotating memtable size={}",
        ...
        let (flushed_wal_number, new_wal_number) = {
            ...
            wal_guard.rotate().map_err(|e| {
                Error::Other(format!(
                    "Failed to rotate WAL before memtable rotation: {}",
                    e
                ))
            })?;
        ...
        // Get table ID before taking immutable_memtables lock.
        // Lock ordering must stay consistent with flush path:
        // level_manifest -> immutable_memtables.
        let table_id = self.level_manifest.read()?.next_table_id();
        // Track immutable memtable with its WAL number
    ...
    pub(crate) fn flush_all_immutables_sync(&self) -> Result<()> {
        ...
        if count > 0 {
            log::debug!(
                "flush_all_immutables_sync: flushed {} immutable memtables",
                count
            );
        }
        Ok(())
    ...
    fn flush_memtable_and_update_manifest(
        ...
        // Get table ID before taking immutable_memtables lock.
        // This preserves lock ordering with flush_and_update_manifest:
        // level_manifest -> immutable_memtables.
        let table_id = self.level_manifest.read()?.next_table_id();
    ...
    fn flush_all_memtables_for_shutdown(&self) -> Result<()> {
        ...
        for entry in immutables_to_flush {
            if entry.memtable.is_empty() {
                ...
                log::debug!(
                    "Skipped empty immutable memtable: table_id={}",
                    entry.table_id
                );
                continue;
            }
...
impl LsmCommitEnv {
    ...
    fn rotate_memtable_and_wake(&self) -> Result<()> {
    ...
    /// Apply a batch one entry at a time so oversized transactions can span
    /// multiple memtables without failing commit.
    fn apply_with_rotations(&self, batch: &Batch) -> Result<()> {
        for (idx, entry, current_seq_num, timestamp) in batch.entries_with_seq_nums()? {
            let mut single = Batch::new(current_seq_num);
            let valueptr = batch.valueptrs.get(idx).cloned().flatten();
            single.add_record_with_valueptr(
                entry.kind,
                entry.key.clone(),
                entry.value.clone(),
                valueptr,
            ...
            loop {
                ...
                match result {
                    ...
                    Err(Error::ArenaFull) => {
                        log::debug!(
                            "apply: arena full during single-entry apply, rotating memtable"
                        );
                        self.rotate_memtable_and_wake()?;
                    }
                    Err(e) => return Err(e),
                }
            }
...
impl CommitEnv for LsmCommitEnv {
    ...
    fn apply(&self, batch: &Batch) -> Result<()> {
        self.apply_with_rotations(batch)
    }
...
pub(crate) struct Core {
    ...
    /// Write admission lock used for operations that require a linearizable
    /// check+write decision (e.g., put_if_not_exists).
    pub(crate) write_admission: tokio::sync::Mutex<()>,
...
impl Core {
    ...
    pub(crate) fn new(opts: Arc<Options>) -> Result<Self> {
        ...
        let commit_env = Arc::new(LsmCommitEnv::new(
            Arc::clone(&inner),
        ...
        let core = Self {
            ...
            write_admission: tokio::sync::Mutex::new(()),
    ...
    pub(crate) async fn commit(&self, batch: Batch, sync: bool) -> Result<()> {
        let _write_guard = self.write_admission.lock().await;
        self.commit_pipeline.commit(batch, sync).await
    }

    pub(crate) async fn commit_without_admission(&self, batch: Batch, sync: bool) -> Result<()> {
        self.commit_pipeline.commit(batch, sync).await
    }
    ...
    pub async fn close(&self) -> Result<()> {
        ...
        log::debug!(
            "Cleaning up obsolete WAL files (min_wal_to_keep={})",
            min_wal_to_keep
        );
...
impl Tree {
    ...
    /// Returns VLog GC telemetry for this database instance.
    pub fn vlog_gc_metrics(&self) -> crate::VLogGcMetrics {
        self.core
            .vlog
            .as_ref()
            .map(|vlog| vlog.gc_metrics_snapshot())
            .unwrap_or_default()
    }

    /// Forces a synchronous maintenance cycle.
    ///
    ...
    pub fn force_maintenance(&self) -> Result<()> {
        ...
        let min_oldest_vlog = self
            .core
            .inner
            .level_manifest
            .read()?
            .min_oldest_vlog_file_id();
        let min_oldest_vlog = resolve_vlog_cleanup_floor(
            min_oldest_vlog,
    ...
    pub async fn merge<K>(&self, key: K, op: MergeOp, missing: MergeMissingBehavior) -> Result<()>
    where
        K: IntoBytes,
    {
        ...
        if missing == MergeMissingBehavior::ErrorIfMissing {
            let snapshot = Snapshot::new(Arc::clone(&self.core), self.core.seq_num());
            let exists = snapshot.get(&key_bytes)?.is_some();
            drop(snapshot);
            if !exists {
        ...
        let delta = match op {
            MergeOp::Add(value) => value,
            MergeOp::Sub(value) => -value,
        }
        .to_le_bytes()
        .to_vec();

        let mut batch = Batch::new(0);
        let ts = self.core.inner.opts.clock.now();
        batch.add_record(InternalKeyKind::Merge, key_bytes, Some(delta), ts)?;
        self.core.commit_without_admission(batch, false).await
    }

    /// Inserts a key only if it does not exist in the latest committed state.
    ///
    /// This operation is linearizable with respect to other commits.
    pub async fn put_if_not_exists<K, V>(&self, key: K, value: V) -> Result<PutIfAbsentResult>
    where
        K: IntoBytes,
        V: IntoBytes,
    {
        ...
        let mut batch = Batch::new(0);
        let ts = self.core.inner.opts.clock.now();
        batch.add_record(InternalKeyKind::Set, key_bytes, Some(value_bytes), ts)?;
        self.core.commit_without_admission(batch, false).await?;
        Ok(PutIfAbsentResult::Inserted)
    }

    /// Deletes keys in half-open range [start, end) by writing a range tombstone.
    pub async fn delete_range<K>(&self, start: K, end: K) -> Result<()>
    where
        K: IntoBytes,
    {
    ...
    pub fn restore_from_checkpoint<P: AsRef<Path>>(
        ...
        let (wal_seq_num_opt, recovered_memtable) = Core::replay_wal_with_repair(
            ...
            |memtable, wal_number| {
                ...
                self.core
                    .inner
                    .flush_and_update_manifest(&memtable, table_id, wal_number)?;
                log::info!(
...
impl TreeBuilder {
    ...
    /// Sets per-level bloom filter bits-per-key (Monkey-style allocation).
    pub fn with_bloom_bits_per_key_per_level(mut self, bits: Vec<usize>) -> Self {
        self.opts = self.opts.with_bloom_bits_per_key_per_level(bits);
        self
    }
        ...
        if let Ok(location) = ValueLocation::decode(encoded_value) {
            if location.is_value_pointer() {
                if let Ok(pointer) = ValuePointer::decode(&location.value) {
                    min_file_id = Some(
                        min_file_id.map_or(pointer.file_id, |current| current.min(pointer.file_id)),
                    );
                }
            }
        }
...
pub(crate) fn cleanup_vlog_and_index(
    ...
    if let Some(ref vlog) = vlog {
        if let Err(e) = vlog.cleanup_obsolete_files(min_oldest_vlog) {
            log::warn!(
                "Failed to cleanup obsolete vlog files during {}: {}",
                context,
                e
            );
    ...
    if let Err(e) = cleanup_stale_versioned_index(versioned_index, min_oldest_vlog) {
        log::warn!(
            "Failed to cleanup stale versioned_index entries during {}: {}",
            context,
            e
        );
        // Don't propagate error - cleanup failures shouldn't fail the primary operation
