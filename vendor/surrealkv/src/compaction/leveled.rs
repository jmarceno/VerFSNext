...
pub(crate) struct Strategy {
    ...
    // Number of top levels using tiered behavior
    compaction_tiered_levels: u8,
...
impl Default for Strategy {
    fn default() -> Self {
        ...
        Self {
            ...
            compaction_tiered_levels: opts.compaction_tiered_levels,
...
impl Strategy {
    ...
    pub(crate) fn select_tables_for_compaction(
        ...
        if source_level_num == 0 || self.is_tiered_level(source_level_num) {
            // Tiered source levels compact whole runs.
        ...
        let source_tables: Vec<_> = if source_level_num == 0 {
            ...
            source_level
                .tables
                .iter()
                .filter(|t| table_id_set.contains(&t.id))
                .collect()
        };
    ...
    fn is_tiered_level(&self, level: u8) -> bool {
        level < self.compaction_tiered_levels
    }
    ...
    fn compute_compaction_scores(&self, manifest: &LevelManifest) -> Vec<(u8, f64)> {
        ...
        let tiered_levels = self
            .compaction_tiered_levels
            .clamp(1, manifest.last_level_index().saturating_add(1));
        ...
        // Tiered top levels (except L0): score considers run/file count and bytes.
        for level in 1..tiered_levels {
            let l = &levels[level as usize];
            let file_score = l.tables.len() as f64 / self.level0_file_num_trigger as f64;
            let bytes = Self::level_bytes(l);
            let target = self.target_bytes_for_level(level);
            let byte_score = if target > 0 {
                bytes as f64 / target as f64
            } else {
                0.0
            };
            let score = file_score.max(byte_score);
            if score >= 1.0 {
                scores.push((level, score));
            }
        }

        // Leveled bottom levels: score = level_bytes / target_bytes.
        for level in 1..=manifest.last_level_index() {
            if level < tiered_levels {
                continue;
            }
