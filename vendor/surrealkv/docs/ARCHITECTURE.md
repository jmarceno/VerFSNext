...
## Merge, PutIfNotExists, and Range Tombstones

SurrealKV supports three LSM-native mutation types for counter workloads, optimistic insertions, and bulk key removal.

### Merge Operators

- API: `Tree::merge(key, MergeOp::{Add,Sub}, MergeMissingBehavior)` and transaction-level `merge(...)`.
- Merge payload encoding: fixed-width little-endian `i64` delta (`+N` / `-N`).
- Counter base encoding: fixed-width little-endian `i64` (stored as inline `ValueLocation`).
- Missing-key behavior:
  - `TreatMissingAsZero` (default for counters): merge chain resolves against base `0`.
  - `ErrorIfMissing`: operation fails if key is absent at decision time.
- Read resolution order for point get:
  1. Newest visible versions first by sequence number.
  2. Accumulate `Merge` deltas.
  3. Stop at `Set`/`Replace` base, `Delete`, `RangeDelete`, or end-of-history.
  4. If no base is found, resolve with base `0`.

### PutIfNotExists

- API: `Tree::put_if_not_exists(key, value) -> PutIfAbsentResult::{Inserted,AlreadyExists}`.
- Semantics: single-winner insert under concurrency.
...
- Boundary semantics: half-open interval `[start, end)` in lexicographic byte order.
...
- Internal representation:
  ...
  - kind = `RangeDelete`
- Read behavior:
  - Point gets suppress keys covered by a newer `RangeDelete`.
  - Iterators return only keys whose final resolved point state remains visible after range tombstone checks.

### Compaction Interaction

- Merge chains are preserved safely during compaction.
- When safe (no active snapshot/version-retention boundary), merge chains are folded into a single `Set` value.
- If folding cannot be proven safe, merge chain entries are retained to preserve correctness.
...
SurrealKV supports Monkey-style level-aware Bloom allocation via `bloom_bits_per_key_per_level`.
At SST creation time, the target level selects its bits-per-key from that vector (last value is reused for deeper levels).

Important implementation detail:
- The Bloom policy name and on-disk filter encoding remain unchanged (`leveldb.BloomFilter`)
- Only the bits-per-key budget is changed per level

This is intentional. Filter lookup in table metadata is keyed by policy name (`filter.<name>`). Keeping a stable name/format avoids read-compatibility hazards while still enabling level-aware false-positive tuning.
...
SurrealKV uses a **hybrid tiered+leveled** strategy:
- Top levels `[0, compaction_tiered_levels)` use tiered behavior (higher merge fan-in, overlap allowed)
- Lower levels use leveled behavior (non-overlapping key ranges, overlap-based merges)

Each level still uses size targets, with each level approximately 10x larger than the previous:
...
│    Tiered Ln score = max(file_count / trigger, level_bytes / target_bytes)  │
│    Leveled Ln score = level_bytes / target_bytes_for_level                  │
│                                                                             │
...
│                                                                             │
│  3. If target is leveled: select overlapping files from target level        │
│     If target is tiered: compact source runs without target overlap pull    │
└─────────────────────────────────────────────────────────────────────────────┘
                                       │
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
**Tiered Top-Level Score (N > 0, N < `compaction_tiered_levels`):**
```
Ln_score = max(file_count / trigger, level_bytes / target_bytes_for_level)
```

**Leveled Level Score (N >= `compaction_tiered_levels`):**
```
Ln_score = level_bytes / target_bytes_for_level
```
Leveled levels are compacted when they exceed their target size.
...
After selecting source files:
1. Tiered source levels compact full source runs
2. Leveled source levels expand by boundary-sharing source files
3. Overlapping target files are included only when compacting into leveled levels

### Background Scheduling

Level compaction runs as an independent background worker and is not gated by the background GC toggle. This prevents compaction starvation when GC is disabled.
...
Rotation policy:
- Rotate before append if `current_size + entry_size > vlog_max_file_size` and the current file already contains at least one entry.
- Rotate when `current_size >= vlog_max_file_size`.
- A single entry larger than `vlog_max_file_size` is written atomically to a fresh file (entries are not split across files), so that one file may exceed the configured max.
```

VLog values are compressed before writing, using the per-file compression type stored in the VLog header. The default vlog compression is `lz4`.
...
| `vlog_compression` | `lz4` | Compression used for values stored in VLog files |
| `vlog_gc_discard_ratio` | 0.5 | Trigger GC when 50% of file is garbage |

SSTable block compression defaults to `snappy` on all levels (`compression_per_level = [SnappyCompression]`).
...
2. **Rotation**: The active file rotates before an append that would overflow a non-empty file, keeping normal files bounded by `vlog_max_file_size`; single oversized entries are allowed in a fresh file
