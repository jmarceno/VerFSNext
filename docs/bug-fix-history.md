...
3. **No batcher drain before read** (`src/fs/fuse.rs:705`): The FUSE `read()` handler never drained the write batcher. With FUSE writeback cache enabled (`FUSE_WRITEBACK_CACHE` in `vendor/async-fusex/src/session.rs:97`), fire-and-forget writes (`FUSE_WRITE_CACHE` flag) could be pending in the batcher while a read returned stale data.
