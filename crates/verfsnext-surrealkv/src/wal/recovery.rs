...
impl Reporter for DefaultReporter {
    ...
    fn old_log_record(&mut self, bytes: usize) {
        log::warn!(
            "Old log record encountered in WAL {}: {} bytes",
            self.log_number,
            bytes
        );
...
mod tests {
    ...
    fn test_replay_wal_sequence_number_tracking() {
        ...
        assert_eq!(
            memtables.len(),
            2,
            "Should create one memtable per WAL segment"
        );
    ...
    fn test_replay_wal_empty_directory() {
        ...
        assert_eq!(
            max_seq_num_opt, None,
            "Empty WAL directory should return None"
        );
        assert_eq!(
            memtables.len(),
            0,
            "Empty WAL directory should return no memtables"
        );
    }
    ...
    fn test_replay_wal_multiple_batches() {
        ...
        assert_eq!(
            entry_count, 4,
            "Memtables should contain all 4 entries from both WAL segments"
        );
    }
    ...
    fn test_post_corruption_replay() {
        ...
        assert_eq!(
            max_seq_num, None,
            "Should have recovered None when first record is corrupted"
        );

        // Verify that no memtable was returned (since first record was corrupted)
        assert!(
            memtable_opt.is_none(),
            "Should have no memtable when first record is corrupted"
        );
    }
    ...
    fn test_repair_with_three_batches_corrupt_third() {
        ...
        file.seek(std::io::SeekFrom::Start(third_batch_offset))
            .unwrap();
        let record_type = 3u8;
        let length = 10u16;
        ...
        if let Some(memtable) = memtable_opt {
            ...
            assert_eq!(
                entry_count, 4,
                "Should have recovered 4 entries from first two batches"
            );
        } else {
    ...
    fn test_default_reporter_usage() {
        ...
        assert_eq!(
            memtables.len(),
            1,
            "Should create one memtable for single segment"
        );
