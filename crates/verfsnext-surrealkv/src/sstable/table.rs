...
pub(crate) struct TableWriter<W: Write> {
    ...
    /// Filter policy chosen for this table at creation time.
...
impl<W: Write> TableWriter<W> {
    fn select_filter_policy(opts: &Options, target_level: u8) -> Option<Arc<dyn FilterPolicy>> {
        //TODO: (jmarceno) Fix this as we do not have legacy to worry about.
        // Monkey minimal strategy:
        // keep the on-disk filter format and policy name stable
        // (`leveldb.BloomFilter`) while selecting level-specific bits-per-key.
        // This avoids read-compat risks because filter lookup is keyed by policy
        // name in the meta index.
        if !opts.bloom_bits_per_key_per_level.is_empty() && opts.filter_policy.is_some() {
            let idx = (target_level as usize).min(opts.bloom_bits_per_key_per_level.len() - 1);
            let bits = opts.bloom_bits_per_key_per_level[idx];
            return Some(Arc::new(LevelDBBloomFilter::new(bits)));
        }
    ...
    pub(crate) fn new(writer: W, id: u64, opts: Arc<Options>, target_level: u8) -> Self {
        let selected_filter_policy = Self::select_filter_policy(&opts, target_level);
        let fb = {
            if let Some(policy) = selected_filter_policy.clone() {
        ...
        TableWriter {
            ...
            filter_block: fb,
            filter_policy: selected_filter_policy,
    ...
    pub(crate) fn add(&mut self, key: InternalKey, val: &[u8]) -> Result<()> {
        ...
        if self.filter_block.is_none() {
            if let Some(filter_policy) = self.filter_policy.as_ref() {
        ...
        if let Ok(location) = ValueLocation::decode(val) {
            if location.is_value_pointer() {
                if let Ok(pointer) = ValuePointer::decode(&location.value) {
                    ...
                    self.min_vlog_file_id = Some(
                        self.min_vlog_file_id
                            .map_or(file_id, |min| min.min(file_id)),
                    );
                }
            }
        }
    ...
    fn write_data_block(&mut self, next_key: &[u8]) -> Result<()> {
        ...
        let compression_type = self
            .compression_selector
            .select_compression(self.target_level);
        ...
        self.partitioned_index
            .add(&separator_key, &handle_encoded)?;
    ...
    pub(crate) fn finish(mut self) -> Result<usize> {
        ...
        let (ix_handle, new_offset) =
            self.partitioned_index
                .finish(&mut self.writer, compression_type, self.offset)?;
...
pub(crate) fn compress_block(
    ...
    match compression {
        ...
        CompressionType::Lz4 => Ok(lz4_flex::compress_prepend_size(&raw_block)),
...
pub(crate) fn decompress_block(
    ...
    match compression {
        ...
        CompressionType::Lz4 => lz4_flex::decompress_size_prepended(compressed_block)
            .map_err(|e| Error::Decompression(e.to_string())),
...
impl Table {
    ...
    pub(crate) fn read_block(&self, location: &BlockHandle) -> Result<Arc<Block>> {
        ...
        if let Some(block) = self
            .opts
            .block_cache
            .get_data_block(self.id, location.offset() as u64)
        {
            return Ok(block);
        }
        ...
        self.opts
            .block_cache
            .insert_data_block(self.id, location.offset() as u64, Arc::clone(&b));
    ...
    pub(crate) fn read_block_with_comparator(
        ...
        if let Some(block) = self
            .opts
            .block_cache
            .get_data_block_history(self.id, location.offset() as u64)
        {
            return Ok(block);
        }
    ...
    pub(crate) fn is_key_in_key_range(&self, key: &InternalKey) -> bool {
        ...
        self.opts
            .comparator
            .compare(key.user_key.as_slice(), &smallest.user_key)
            >= Ordering::Equal
            && self
                .opts
                .comparator
                .compare(key.user_key.as_slice(), &largest.user_key)
                <= Ordering::Equal
    }
    ...
    pub(crate) fn is_after_range(&self, range: &InternalKeyRange) -> bool {
        ...
        match &range.1 {
            ...
            Bound::Included(k) => {
                ...
                self.opts
                    .comparator
                    .compare(&smallest.user_key, &k.user_key)
                    == Ordering::Greater
            }
            Bound::Excluded(k) => {
...
impl<'a> TableIterator<'a> {
    ...
    fn init_data_block(&mut self) -> Result<()> {
        ...
        let iter = if let Some(ref comparator) = self.custom_comparator {
            let block = self
                .table
                .read_block_with_comparator(&handle, Arc::clone(comparator))?;
            block.iter()?
        } else {
        ...
        match &self.range.0 {
            Bound::Included(start) => {
                ...
                self.table
                    .opts
                    .comparator
                    .compare(user_key, &start.user_key)
                    != Ordering::Less
            }
