...
impl fmt::Display for SSTableError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            // Block errors
            SSTableError::BlockTooSmall { size, min_size } => {
                write!(
                    f,
            ...
            SSTableError::FailedToDecodeRestartCount { block_size } => {
                write!(
                    f,
            ...
            SSTableError::InvalidRestartCount { count, block_size } => {
                write!(
                    f,
            ...
            SSTableError::FailedToDecodeRestartPoint { index, offset } => {
                write!(
                    f,
            ...
            SSTableError::FailedToDecodeSharedPrefix { offset } => {
                write!(
                    f,
            ...
            SSTableError::FailedToDecodeNonSharedKeyLength { offset } => {
                write!(
                    f,
            ...
            SSTableError::FailedToDecodeValueSize { offset } => {
            ...
            SSTableError::IntegerOverflowKeyEnd { offset } => {
                write!(
                    f,
            ...
            SSTableError::IntegerOverflowValueEnd { offset } => {
                write!(
                    f,
            ...
            SSTableError::BadMagicNumber { magic } => {
                write!(f, "invalid table (bad magic number: {:x?})", magic)
            }
            SSTableError::FooterTooShort { footer_len } => {
                write!(f, "invalid table (footer too short): {}", footer_len)
            }
            SSTableError::InvalidChecksumType { checksum_type } => {
            ...
            SSTableError::ChecksumVerificationFailed { block_offset } => {
                write!(
                    f,
