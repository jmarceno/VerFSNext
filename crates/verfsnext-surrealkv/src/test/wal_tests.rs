...
fn test_wal_replay_all_segments() {
    ...
    assert_eq!(
        sequence_number, 304,
        "Expected max sequence number from all segments"
    );

    // Verify the memtable contains data from ALL segments (3 + 4 + 5 = 12 entries)
    assert_eq!(
        entry_count, 12,
        "Expected 12 entries from all 3 segments combined"
    );
}
...
fn test_record_type_from_u8() {
    ...
    assert_eq!(
        RecordType::from_u8(9).unwrap(),
        RecordType::SetCompressionType
    );
...
fn test_cleanup_respects_min_wal_number() {
    ...
    assert_eq!(
        segment_ids_before.len(),
        5,
        "Should have 5 segments initially"
    );
    ...
    assert_eq!(
        remaining_segment_ids.len(),
        2,
        "Should have 2 segments remaining"
    );
