...
async fn test_file_count_after_recovery() {
    ...
    // Reopen and verify file counts
    {
        ...
        assert!(
            sst_count_after >= 1,
            "SST files should exist after recovery"
        );
...
async fn test_wal_cleanup_after_recovery_without_flush() {
    ...
    let wal_count_before_flush;
    {
        ...
        assert!(
            wal_count_before_flush > 0,
            "WAL files should exist after recovery"
        );
...
async fn test_manifest_log_number_progression() {
    ...
    // Cycle 1: Write and flush
    {
        ...
        for i in 0..10 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("c1_key_{}", i).as_bytes(), b"cycle1")
                .unwrap();
            txn.commit().await.unwrap();
        }
        ...
        assert!(
            log_num_after_flush > log_num_initial,
            "Log number should advance after flush"
        );
    ...
    // Cycle 2: Reopen, write and flush
    {
        ...
        for i in 0..10 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("c2_key_{}", i).as_bytes(), b"cycle2")
                .unwrap();
            txn.commit().await.unwrap();
        }
        ...
        assert!(
            log_num_after > log_num_before,
            "Log number should advance again"
        );
    ...
    // Cycle 3: Reopen, write and flush again
    {
        ...
        for i in 0..10 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("c3_key_{}", i).as_bytes(), b"cycle3")
                .unwrap();
            txn.commit().await.unwrap();
        }
        ...
        assert!(
            log_num_after > log_num_before,
            "Log number should advance with flush in cycle 3"
        );
...
async fn test_corrupted_wal_with_valid_sst() {
...

    {
        ...
        for i in 0..10 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("sst_key_{}", i).as_bytes(), b"safe_in_sst")
                .unwrap();
            txn.commit().await.unwrap();
        }
        tree.flush().unwrap();
        ...
        for i in 0..5 {
            let mut txn = tree.begin().unwrap();
            txn.set(format!("wal_key_{}", i).as_bytes(), b"in_wal")
                .unwrap();
            txn.commit().await.unwrap();
        }
