...
fn test_sequential_5_segments_recovery() {
    ...
    assert_eq!(
        final_seq, 1500,
        "Final sequence should be 1500 (1000 + 500 entries)"
    );
...
fn test_single_large_vs_multiple_small_segments() {
    ...
    log::info!(
        "Single segment: create={:?}, replay={:?}",
        single_create,
        single_replay
    );
    log::info!(
        "Multiple segments: create={:?}, replay={:?}",
        multi_create,
        multi_replay
    );
}
...
fn test_crash_immediately_after_rotation() {
    ...
    assert_eq!(
        max_seq_opt,
        Some(149),
        "Should recover all 50 entries from segment 0"
    );
...
fn test_all_empty_segments() {
    ...
    assert!(
        memtables.is_empty(),
        "Should have no memtables for empty segments"
    );
...
fn test_corruption_first_record_segment_0() {
    ...
    match result {
        Err(Error::WalCorruption { segment_id, .. }) => {
...
fn test_truncated_wal_file() {
    ...
    match result {
        Err(Error::WalCorruption { segment_id, .. }) => {
            ...
            assert!(
                segment_id >= 1,
                "Corruption should be in segment 1 or later"
            );
            // Corruption stops recovery immediately - no memtables returned
        }
        Ok((_, memtables)) => {
            ...
            assert!(
                entry_count >= 30,
                "Should have at least segment 0, got {}",
                entry_count
            );
        }
        Err(e) => panic!("Unexpected error: {}", e),
...
fn test_all_segments_already_flushed() {
    ...
    assert!(
        memtables.is_empty(),
        "Should have no memtables when all segments already flushed"
    );
    assert_eq!(WalTestHelper::count_total_entries(&memtables), 0);
}
...
fn test_very_large_values_across_segments() {
    ...
    for seg_idx in 0..5 {
        ...
        for i in 0..5 {
            ...
            batch
                .set(key.as_bytes().to_vec(), large_value.clone(), 0)
                .unwrap();
        }
        wal.append(&batch.encode().unwrap()).unwrap();
...
fn test_many_small_batches() {
    ...
    for seg_idx in 0..10 {
        for batch_idx in 0..100 {
            ...
            batch
                .set(key.as_bytes().to_vec(), b"value".to_vec(), 0)
                .unwrap();
            wal.append(&batch.encode().unwrap()).unwrap();
            entry_count += 1;
        }
...
fn test_symlink_to_wal_segment() {
    ...
    #[cfg(unix)]
    {
        ...
        assert!(
            entry_count >= 42,
            "Should process at least original segment"
        );
    }
}
...
fn test_concurrent_wal_directory_modifications() {
    ...
    match result {
        ...
        Ok((_, memtables)) => {
            ...
            assert!(
                entry_count >= 90,
                "Should have recovered segments 0-1, got {}",
                entry_count
            );
        }
        Err(e) => panic!("Unexpected error: {}", e),
...
fn test_compressed_wal_segments() {
    ...
    for seg_idx in 0..3 {
        ...
        for i in 0..50 {
            ...
            batch
                .set(key.as_bytes().to_vec(), value.as_bytes().to_vec(), 0)
                .unwrap();
        }
        wal.append(&batch.encode().unwrap()).unwrap();
...
fn test_mixed_compression_segments() {
    ...
    for seg_idx in 0..2 {
        ...
        for i in 0..40 {
            ...
            batch
                .set(key.as_bytes().to_vec(), b"uncompressed_value".to_vec(), 0)
                .unwrap();
        }
        wal.append(&batch.encode().unwrap()).unwrap();
        wal.rotate().unwrap();
    ...
    for seg_idx in 0..2 {
        ...
        for i in 0..40 {
            ...
            batch
                .set(key.as_bytes().to_vec(), b"compressed_value".to_vec(), 0)
                .unwrap();
        }
        wal.append(&batch.encode().unwrap()).unwrap();
        if seg_idx < 1 {
...
fn test_compressed_data_corruption() {
    ...
    for i in 0..60 {
        ...
        batch
            .set(
                key.as_bytes().to_vec(),
                b"compressed_value_data".to_vec(),
                0,
            )
            .unwrap();
    }
