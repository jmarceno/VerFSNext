        ...
        CompactionChoice::Merge(input) => {
            ...
            if input.source_level == 3 {
                assert_eq!(
                    ...
                    "Bottom level should compact to itself"
                );
            }
        }
    }
...
fn test_select_tables_for_compaction_bug() {
    ...
    let selected_tables = strategy
        .select_tables_for_compaction(&source_level, &next_level, 0, false)
        .unwrap();
...
fn test_l1_to_l2_table_selection() {
    ...
    let selected_tables = strategy
        .select_tables_for_compaction(&source_level, &next_level, 1, false)
        .unwrap();
...
fn test_l1_compaction_bounds_correctness() {
    ...
    let selected_tables = strategy
        .select_tables_for_compaction(&source_level, &next_level, 1, true)
        .unwrap();
    ...
    assert_eq!(
        selected_source_ids.len(),
        1,
        "Should select exactly ONE L1 table"
    );
    ...
    let selected_l2_ids: HashSet<_> = selected_tables
        .iter()
        .filter(|&&id| l2_table_ids.contains(&id))
        .copied()
        .collect();
...
async fn test_l0_tombstone_propagation_overlapping() {
    ...
    for i in 0..20 {
        ...
        entries1.push((
            InternalKey::new(key, 100 + i, InternalKeyKind::Set, 0),
            encoded_value,
        ));
    }
    ...
    for i in [2, 8, 14, 17] {
        let key = format!("key-{i:03}").into_bytes();
        entries3.push((
            InternalKey::new(key, 200 + i / 2, InternalKeyKind::Delete, 0),
            vec![],
        ));
    }
...
fn test_tombstone_propagation_journey() {
    ...
    let has_tombstones = bottom_result
        .iter()
        .any(|(key, _)| key.is_hard_delete_marker());
    ...
    let has_test_key = bottom_result
        .iter()
        .any(|(key, _)| &key.user_key == b"test-key");
...
fn test_table_properties_population() {
    ...
    assert_eq!(
        props.index_partitions, 1,
        "Should have 1 index partition for small table"
    );
    assert!(
    ...
    assert_eq!(
        props.raw_value_size, 675,
        "Raw value size should be tracked"
    );
    assert!(
    ...
    assert_eq!(
        ...
        Some(0),
        "Oldest key time is 0 in test"
    );
    assert_eq!(
        props.newest_key_time,
        Some(0),
        "Newest key time is 0 in test"
    );

    // Verify range deletion metrics
    assert_eq!(
        props.num_range_deletions, 5,
        "Should have 5 range deletions (every 20th key)"
    );
...
async fn test_soft_delete_compaction_behavior() {
    ...
    for table_idx in 0..2 {
        ...
        let table = env
            .create_test_table(100 + table_idx, l0_entries.clone())
            .unwrap();
        Arc::make_mut(&mut levels.get_levels_mut()[0]).insert(table);
    }
    ...
    assert_eq!(
        soft_deletes, 4,
        "Should have exactly 4 soft deletes (keys 0, 3, 6, 9)"
    );
    assert_eq!(
        regular_deletes, 0,
...
async fn test_older_soft_delete_marked_stale_during_compaction() {
    ...
    let l1_entries = vec![
        (
            InternalKey::new(key.clone(), 200, InternalKeyKind::Set, 0),
            encoded_value,
        ),
        (
    ...
    assert_eq!(
        soft_deletes, 1,
        "Should have exactly 1 soft delete (the latest)"
    );
    assert_eq!(
        sets, 0,
        "Should have no Set entries (superseded by SoftDelete)"
    );
}
...
fn test_expand_same_user_key_different_seq() {
    ...
    assert_eq!(
        result.len(),
        2,
        "Both files with same user key should be included"
    );
    assert!(result.contains(&1));
    assert!(result.contains(&2));
}
...
fn test_clean_cut_shared_boundary_key() {
    ...
    assert!(
        selected.contains(&1),
        "File 1 should be included (initially selected)"
    );
    assert!(
        selected.contains(&2),
...
fn test_clean_cut_chain_expansion() {
    ...
    assert!(
        selected.contains(&1),
        "File 1 should be included (chain expansion)"
    );
    assert!(
        selected.contains(&2),
        "File 2 should be included (initially selected)"
    );
    assert!(
        selected.contains(&3),
        "File 3 should be included (chain expansion)"
    );
...
fn test_clean_cut_no_expansion_needed() {
    ...
    assert!(
        selected.contains(&1),
        "File 1 should be the only selected file"
    );
}
