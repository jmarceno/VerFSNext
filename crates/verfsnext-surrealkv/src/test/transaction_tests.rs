...
use crate::{
    Error, Key, LSMIterator, MergeMissingBehavior, MergeOp, Mode, Options, PutIfAbsentResult,
    TreeBuilder, WriteOptions,
};
...
async fn test_keys_method() {
    ...
    // Test with RYOW - add a key in the current transaction
    {
        ...
        assert!(
            !key_names.contains(&"key3".to_string()),
            "key3 should be removed"
        );
    }
}
...
async fn test_versioned_queries_basic() {
    ...
    let v1 = versions
        .iter()
        .find(|(_, _, timestamp, _)| *timestamp == ts1)
        .unwrap();
    let v2 = versions
        .iter()
        .find(|(_, _, timestamp, _)| *timestamp == ts2)
        .unwrap();
...
async fn test_versioned_queries_with_deletes() {
    ...
    let mut iter = tx
        .history_with_options(b"key1", b"key2", &opts_with_tombstones)
        .unwrap();
...
async fn test_range_at_version_with_deletes() {
...

    {
        ...
        let mut iter = tx_before
            .history_with_options(b"key1", b"key4", &opts_with_tombstones)
            .unwrap();
        let scan_before = point_in_time_from_history(&mut iter, ts_after_insert).unwrap();
        assert_eq!(
            scan_before.len(),
            3,
            "Should have all 3 keys before deletes"
        );
    }
    ...
    assert_eq!(
        versions2.len(),
        0,
        "Hard deleted key should have no versions"
    );
...
async fn test_scan_all_versions() {
    ...
    for (key, value, timestamp, is_tombstone) in all_versions {
        key_versions
            .entry(key)
            .or_default()
            .push((value.clone(), timestamp, is_tombstone));
    }
...
mod version_tests {
    ...
    async fn test_scan_all_versions_with_limit_with_multiple_versions_per_key() {
        ...
        for key in unique_keys {
            let key_versions: Vec<_> = all_results
                .iter()
    ...
    async fn test_scan_all_versions_with_batches() {
        ...
        let key1_results: Vec<_> = all_results
            .iter()
            .filter(|(k, _, _, _)| k == b"key1")
            .collect();
        assert_eq!(key1_results.len(), 4);

        // Verify key2 has 2 versions
        let key2_results: Vec<_> = all_results
            .iter()
            .filter(|(k, _, _, _)| k == b"key2")
            .collect();
        assert_eq!(key2_results.len(), 2);

        // Verify key3 has 4 versions
        let key3_results: Vec<_> = all_results
            .iter()
            .filter(|(k, _, _, _)| k == b"key3")
            .collect();
        assert_eq!(key3_results.len(), 4);

        // Verify key4 has 1 version
        let key4_results: Vec<_> = all_results
            .iter()
            .filter(|(k, _, _, _)| k == b"key4")
            .collect();
        assert_eq!(key4_results.len(), 1);

        // Verify key5 has 1 version
        let key5_results: Vec<_> = all_results
            .iter()
            .filter(|(k, _, _, _)| k == b"key5")
            .collect();
    ...
    async fn test_replace_mixed_with_regular_operations() {
        ...
        let txn = store.begin().unwrap();
        assert_eq!(
            txn.get(b"key2").unwrap().unwrap(),
            b"updated_regular_value2"
        );

        // Use replace on key1
        let mut txn = store.begin().unwrap();
        txn.replace(b"key1", b"final_set_with_delete_value1")
            .unwrap();
        txn.commit().await.unwrap();
...
async fn test_versioned_range_survives_memtable_flush() {
    ...
    for i in 1..=3 {
        ...
        tx.set_at(b"key1", value.as_bytes(), i as u64 * 100)
            .unwrap();
        tx.commit().await.unwrap();
    }
    ...
    assert_eq!(
        results.len(),
        3,
        "All 3 versions should survive memtable flush"
    );
    assert_eq!(results[0].1, b"v1");
    assert_eq!(results[1].1, b"v2");
    assert_eq!(results[2].1, b"v3");
...
async fn test_memtable_earliest_seq_tracking() {
    ...
    assert_eq!(
        txn.start_seq_num, 1,
        "Transaction start_seq_num should be 1 after commit"
    );
}

fn decode_i64_counter(bytes: &[u8]) -> i64 {
    assert_eq!(bytes.len(), 8, "counter value must be 8-byte i64");
    let mut raw = [0u8; 8];
...
#[test(tokio::test)]
async fn test_merge_add_sub_missing_zero() {
    let (store, _temp_dir) = create_store();
    ...
    store
        .merge(
            b"counter",
            MergeOp::Add(5),
            MergeMissingBehavior::TreatMissingAsZero,
        )
        .await
        .unwrap();
    store
    ...
    let tx = store.begin().unwrap();
    let raw = tx.get(b"counter").unwrap().unwrap();
    assert_eq!(decode_i64_counter(&raw), 3);
}

#[test(tokio::test)]
async fn test_put_if_not_exists_single_winner() {
    let (store, _temp_dir) = create_store();
    let store = Arc::new(store);

    let mut handles = Vec::new();
    for idx in 0..16 {
        let tree = Arc::clone(&store);
        handles.push(tokio::spawn(async move {
            let value = format!("writer-{idx}").into_bytes();
            tree.put_if_not_exists(b"race-key", value).await
        }));
    }
    ...
    for handle in handles {
        match handle.await.unwrap().unwrap() {
            PutIfAbsentResult::Inserted => inserted += 1,
            PutIfAbsentResult::AlreadyExists => exists += 1,
        }
    }

    assert_eq!(inserted, 1, "exactly one writer must win");
    assert_eq!(exists, 15, "all other writers must observe existing key");
}

#[test(tokio::test)]
async fn test_delete_range_half_open_visibility() {
    let (store, _temp_dir) = create_store();

    {
        let mut tx = store.begin().unwrap();
        tx.set(b"a", b"va").unwrap();
        tx.set(b"b", b"vb").unwrap();
        tx.set(b"c", b"vc").unwrap();
        tx.set(b"d", b"vd").unwrap();
        tx.commit().await.unwrap();
    }

    store.delete_range(b"b", b"d").await.unwrap();

    let tx = store.begin().unwrap();
    assert_eq!(tx.get(b"a").unwrap().unwrap(), b"va".to_vec());
    assert!(tx.get(b"b").unwrap().is_none());
    assert!(tx.get(b"c").unwrap().is_none());
    assert_eq!(tx.get(b"d").unwrap().unwrap(), b"vd".to_vec());
}

#[test(tokio::test)]
async fn test_merge_concurrent_no_lost_updates() {
    let (store, _temp_dir) = create_store();
    let store = Arc::new(store);
    ...
    let tx = store.begin().unwrap();
    let raw = tx.get(b"hot-counter").unwrap().unwrap();
    assert_eq!(decode_i64_counter(&raw), (workers * iterations) as i64);
}
