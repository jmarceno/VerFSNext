...
fn generate_json_like_value(id: usize, size: usize) -> Vec<u8> {
    let base = format!(
        r#"{{"id":{},"name":"user_{}","email":"user{}@example.com","data":"#,
        id, id, id
    );
...
fn build_table_with_compression(
...

    {
        ...
        for (k, v) in data {
            builder
                .add(InternalKey::new(k, 1, InternalKeyKind::Set, 0), &v)
                .unwrap();
        }
...
fn test_compression_size_reduction() {
    ...
    let table_compressed = Arc::new(
        Table::new(
            2,
            opts_compressed,
            wrap_buffer(buffer_compressed),
            size_compressed as u64,
        )
        .unwrap(),
    );
    ...
    while iter_uncompressed.valid() && iter_compressed.valid() {
        assert_eq!(
            iter_uncompressed.key().user_key(),
            iter_compressed.key().user_key()
        );
        assert_eq!(
    ...
    assert_eq!(
        count, 10_000,
        "Both tables should have identical 10k entries"
    );
}
...
fn test_compression_iterator_operations() {
    ...
    assert!(
        prev_key < data[9999].0,
        "Previous key should be less than last key"
    );
...
fn test_compression_large_values() {
    ...
    while iter.valid() {
        ...
        assert_eq!(
            value.len(),
            data[count].1.len(),
            "Value length mismatch at index {}",
            count
        );
        assert_eq!(
            value,
...
async fn test_lsm_compression_10k_keys_with_range_scans() {
    ...
    println!(
        "Inserting {} unique keys with compression enabled",
        keys.len()
    );
    ...
    assert_eq!(
        scanned_count,
        keys.len(),
        "Range scan should return all keys"
    );
    println!(
        "Range scan successfully iterated through all {} keys",
        scanned_count
    );
...
async fn test_lsm_compression_persistence_after_reopen() {
...

    {
        ...
        println!(
            "Phase 1: Inserting {} unique keys with compression...",
            keys.len()
        );
...

    {
        ...
        assert_eq!(
            found_count,
            keys.len(),
            "All keys should be found after reopen"
        );
        ...
        assert_eq!(
            scanned_count,
            keys.len(),
            "Range scan should return all keys after reopen"
        );
        println!(
            "✓ Range scan successfully iterated through all {} keys",
            scanned_count
...
async fn test_lsm_compression_disk_size_comparison() {
    ...
    let tree_compressed = crate::TreeBuilder::with_options(opts_compressed.clone())
        .build()
        .unwrap();
    let tree_uncompressed = crate::TreeBuilder::with_options(opts_uncompressed.clone())
        .build()
        .unwrap();
    ...
    let tree_compressed = crate::TreeBuilder::with_options(opts_compressed)
        .build()
        .unwrap();
    for i in [0, 5000, 9999].iter() {
        ...
        assert!(
            result.is_some(),
            "Key {} should exist after reopening compressed tree",
            i
        );
    }
    tree_compressed.close().await.unwrap();
...
fn test_compression_per_level_options() {
    // Test default options (Snappy for all levels)
    let opts = Options::new();
    assert_eq!(
        opts.compression_per_level,
        vec![CompressionType::SnappyCompression]
    );
    ...
    assert_eq!(
        opts.compression_per_level[1],
        CompressionType::SnappyCompression
    );
    assert_eq!(
        opts.compression_per_level[2],
        CompressionType::SnappyCompression
    );
...
fn test_compression_selector_per_level() {
    ...
    assert_eq!(
        selector.select_compression(1),
        CompressionType::SnappyCompression
    );
    assert_eq!(
        selector.select_compression(2),
        CompressionType::SnappyCompression
    );
    // Higher levels use last configured compression
    assert_eq!(
        selector.select_compression(3),
        CompressionType::SnappyCompression
    );
    assert_eq!(
        selector.select_compression(10),
        CompressionType::SnappyCompression
    );
}
