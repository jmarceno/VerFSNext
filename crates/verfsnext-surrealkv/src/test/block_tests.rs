...
fn test_block_empty() {
    ...
    let mut block_iter = Block::new(blockc, Arc::clone(&o.internal_comparator))
        .iter()
        .unwrap();
...
fn test_block_iter() {
    ...
    let mut block_iter = Block::new(block_contents, Arc::clone(&o.internal_comparator))
        .iter()
        .unwrap();
...
fn test_block_iter_reverse() {
    ...
    let block_contents = builder.finish().unwrap();
    let mut iter = Block::new(block_contents, Arc::clone(&o.internal_comparator))
        .iter()
        .unwrap();
...
fn test_block_seek() {
    ...
    assert_eq!(
        Some((
            block_iter.key().user_key(),
            block_iter.value_encoded().unwrap(),
        )),
...
fn test_block_double_ended_iteration() {
    ...
    let mut forward_iter = Block::new(block_contents.clone(), Arc::clone(&o.internal_comparator))
        .iter()
        .unwrap();
    ...
    // Test backward iteration using prev()
    let mut backward_iter = Block::new(block_contents, Arc::clone(&o.internal_comparator))
        .iter()
        .unwrap();
    ...
    assert_eq!(
        forward_keys,
        backward_keys.iter().rev().cloned().collect::<Vec<_>>()
    );
}
...
fn test_block_prev_edge_cases() {
    ...
    .finish()
    .unwrap();
    let mut empty_iter = Block::new(empty_block, Arc::clone(&o.internal_comparator))
        .iter()
        .unwrap();
...
fn test_block_seek_key_not_found_past_end() {
    ...
    for i in 0..5 {
        ...
        builder
            .add(
                &make_internal_key(key.as_bytes(), InternalKeyKind::Set),
                value.as_bytes(),
            )
            .unwrap();
    }
...
fn test_block_seek_between_keys() {
    ...
    builder
        .add(&make_internal_key(b"apple", InternalKeyKind::Set), b"v1")
        .unwrap();
    builder
        .add(&make_internal_key(b"cherry", InternalKeyKind::Set), b"v2")
        .unwrap();
    builder
        .add(&make_internal_key(b"grape", InternalKeyKind::Set), b"v3")
        .unwrap();
...
fn test_block_seek_same_user_key_different_seq_nums() {
    ...
    assert_eq!(
        iter.key().seq_num(),
        100,
        "Should find newest version seq=100"
    );
}
...
fn test_block_single_restart_point() {
    ...
    builder
        .add(
            &make_internal_key(b"only_key", InternalKeyKind::Set),
            b"only_value",
        )
        .unwrap();
    ...
    assert!(
        !iter.valid(),
        "Should be invalid when seeking past single entry"
    );
}
...
fn test_seek_to_last_empty_block() {
    ...
    assert!(
        !iter.valid(),
        "Empty block iterator should be invalid after seek_to_last"
    );
}
...
fn test_prev_at_first_entry() {
    ...
    assert!(
        !iter.valid(),
        "Iterator should be invalid after prev() at first entry"
    );
}
