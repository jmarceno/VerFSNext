...
async fn test_snapshot_keys_only() {
    ...
    let mut keys_only_iter = tx
        .range(Some("key1".as_bytes()), Some("key6".as_bytes()))
        .unwrap();
    let keys_only_data = collect_snapshot_iter(&mut keys_only_iter).unwrap();
    let keys_only: Vec<(Vec<u8>, Option<Vec<u8>>)> = keys_only_data
        .into_iter()
        .map(|(k, _)| (k.user_key, None))
        .collect();
    ...
    let mut regular_range_iter = tx
        .range(Some("key1".as_bytes()), Some("key6".as_bytes()))
        .unwrap();
    let regular_range_data = collect_snapshot_iter(&mut regular_range_iter).unwrap();
    let regular_range: Vec<_> = regular_range_data
        .into_iter()
        .map(|(k, v)| (k.user_key, Some(v)))
        .collect();
...
async fn test_double_ended_iteration_with_tombstones() {
    ...
    let mut forward_iter1 = tx1_ref
        .range(Some("key1".as_bytes()), Some("key4".as_bytes()))
        .unwrap();
    ...
    let mut backward_iter1 = tx1_ref
        .range(Some("key1".as_bytes()), Some("key4".as_bytes()))
        .unwrap();
    ...
    let mut backward_iter2 = tx2_ref
        .range(Some("key1".as_bytes()), Some("key4".as_bytes()))
        .unwrap();
    ...
    for i in 0..forward_items2.len() {
        assert_eq!(
            forward_items2[i].0,
            backward_items2[forward_items2.len() - 1 - i].0
        );
    }
}
...
async fn test_soft_delete_snapshot_double_ended_iteration() {
    ...
    // Test forward iteration
    {
        ...
        let mut forward_iter = snapshot_ref
            .range(Some("key1".as_bytes()), Some("key6".as_bytes()))
            .unwrap();
        let forward_items = collect_snapshot_iter(&mut forward_iter).unwrap();
    ...
    {
        let snapshot_ref = tx.snapshot.as_ref().unwrap();
        let mut backward_iter = snapshot_ref
            .range(Some("key1".as_bytes()), Some("key6".as_bytes()))
            .unwrap();
        let backward_items = collect_snapshot_reverse(&mut backward_iter).unwrap();
...
async fn test_soft_delete_snapshot_mixed_with_hard_delete() {
    ...
    {
        let tx1_ref = tx1.snapshot.as_ref().unwrap();
        let mut range_iter = tx1_ref
            .range(Some("key1".as_bytes()), Some("key5".as_bytes()))
            .unwrap();
...
async fn test_double_ended_iteration_mixed_operations() {
    ...
    let snapshot_ref = tx.snapshot.as_ref().unwrap();
    let mut forward_iter = snapshot_ref
        .range(Some("key01".as_bytes()), Some("key11".as_bytes()))
        .unwrap();
    let forward_items = collect_snapshot_iter(&mut forward_iter).unwrap();
...
fn test_level0_tables_before_range_skipped() {
    ...
    let iter_state = create_iter_state_with_tables(vec![table1, table2, table3], vec![], vec![]);
    ...
    assert_eq!(
        count, 0,
        "No tables should be included as all are before range"
    );
}
...
fn test_level0_tables_after_range_skipped() {
    ...
    assert_eq!(
        count, 0,
        "No tables should be included as all are after range"
    );
}
...
fn test_level0_mixed_overlap_scenarios() {
    ...
    let iter_state =
        create_iter_state_with_tables(vec![table1, table2, table3, table4, table5], vec![], vec![]);
    ...
    assert!(
        count > 0,
        "Should have items from overlapping tables in range"
    );
}
...
fn test_level1_binary_search_correct_range() {
    ...
    let iter_state =
        create_iter_state_with_tables(vec![], vec![table1, table2, table3, table4, table5], vec![]);
...
fn test_level1_query_before_all_tables() {
    ...
    let iter_state = create_iter_state_with_tables(vec![], vec![table1, table2, table3], vec![]);
    ...
    assert_eq!(
        count, 0,
        "No tables should be included as query is before all L1 tables"
    );
}
...
fn test_level1_query_after_all_tables() {
    ...
    assert_eq!(
        count, 0,
        "No tables should be included as query is after all L1 tables"
    );
}
...
fn test_bound_included_start_and_end() {
    ...
    let mut iter =
        KMergeIterator::new_from(iter_state, internal_range, opts.compaction_tiered_levels);
...
fn test_bound_unbounded_start() {
    ...
    assert!(
        count > 0,
        "Should iterate from beginning with unbounded start"
    );
}
...
fn test_fully_unbounded_range() {
    ...
    let iter = KMergeIterator::new_from(
        iter_state,
        (Bound::Unbounded, Bound::Unbounded),
        opts.compaction_tiered_levels,
    );

    let count = count_kmerge_items(iter);
    assert!(
        count > 0,
        "Should return all keys with fully unbounded range"
    );
}
...
async fn test_cache_effectiveness_with_range_query() {
    ...
    assert_eq!(
        first_results.len(),
        10_000,
        "First query should return all 10,000 items"
    );
    assert_eq!(
        second_results.len(),
        10_000,
        "Second query should return all 10,000 items"
    );
    assert!(
...
async fn test_snapshot_iterator_seq_num_filtering_via_transactions() {
    ...
    // Test snapshot1 - should only see key1
    {
        ...
        let mut range_iter = snap_ref
            .range(Some("key0".as_bytes()), Some("key9".as_bytes()))
            .unwrap();
        let range = collect_snapshot_iter(&mut range_iter).unwrap();
