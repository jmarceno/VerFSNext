    ...
    fn rebuild_min_heap(&mut self) {
        ...
        for i in 0..children.len() {
            if children[i].iter.valid() {
                self.min_heap
                    .push(i, |a, b| Self::cmp_min(children, cmp, a, b));
            }
        }
    }
        ...
        match self.direction {
            Direction::Forward => {
                ...
                if valid {
                    ...
                    self.min_heap
                        .sift_down_root(|a, b| Self::cmp_min(children, cmp, a, b));
                } else {
            ...
            Direction::Backward => {
                ...
                if valid {
                    ...
                    self.max_heap
                        .as_mut()
                        .unwrap()
                        .pop(|a, b| Self::cmp_max(children, cmp, a, b));
                }
...
impl<'a> CompactionIterator<'a> {
    ...
    fn decode_inline_counter(value: &[u8]) -> Option<i64> {
        let loc = ValueLocation::decode(value).ok()?;
        if loc.is_value_pointer() || loc.value.len() != 8 {
            return None;
        }

        let mut bytes = [0u8; 8];
        bytes.copy_from_slice(&loc.value);
        Some(i64::from_le_bytes(bytes))
    }

    fn try_fold_merge_chain(&self) -> Option<(InternalKey, Value)> {
        if self.enable_versioning || !self.snapshots.is_empty() {
            return None;
        }
        ...
        let has_merge = self
            ...
            .iter()
            .any(|(key, _)| key.kind() == InternalKeyKind::Merge);
        if !has_merge {
            return None;
        }

        let first = self.accumulated_versions.first()?.0.clone();
        let mut total: i128 = 0;
        let mut saw_merge = false;

        for (key, value) in &self.accumulated_versions {
            match key.kind() {
                InternalKeyKind::Merge => {
                    let delta = Self::decode_inline_counter(value)? as i128;
                ...
                InternalKeyKind::Set | InternalKeyKind::Replace => {
                    ...
                    let base = Self::decode_inline_counter(value)? as i128;
        ...
        let folded_key = InternalKey::new(
        ...
        let folded_value =
            ValueLocation::with_inline_value(total_i64.to_le_bytes().to_vec()).encode();

        Some((folded_key, folded_value))
    }
    ...
    fn process_accumulated_versions(&mut self) -> Result<()> {
        ...
        self.accumulated_versions
            .sort_by_key(|b| std::cmp::Reverse(b.0.seq_num()));
        ...
        if has_merge {
            if let Some(folded) = self.try_fold_merge_chain() {
                self.output_versions.push(folded);
            } else {
                self.output_versions
                    .extend(self.accumulated_versions.iter().cloned());
            }
        ...
        let has_set_with_delete = self
            .accumulated_versions
            .iter()
            .any(|(key, _)| key.is_replace());
