...
impl LevelManifest {
    ...
    pub(crate) fn apply_changeset(
        ...
        for (level, table) in &changeset.new_tables {
            ...
            let largest = table
                .meta
                .largest_seq_num
                .expect("table must have largest_seq_num");
        ...
        self.snapshots
            .retain(|snapshot| !changeset.deleted_snapshots.contains(&snapshot.seq_num));
    ...
    pub(crate) fn revert_changeset(&mut self, rollback: ChangeSetRollback) {
        ...
        self.snapshots
            .retain(|s| !rollback.added_snapshot_seqs.contains(&s.seq_num));
