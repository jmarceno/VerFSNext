...
impl TaskManager {
    pub(crate) fn new(core: Arc<dyn CompactionOperations>, opts: Arc<Options>) -> Self {
        ...
        // Compaction is independent from background GC controls.
        let core = Arc::clone(&core);
        ...
        let handle = tokio::spawn(async move {
            loop {
                ...
                if stop_flag_worker.load(Ordering::SeqCst) {
                    break;
                }
                ...
                loop {
                    match core.compact(Arc::clone(&strategy)) {
                        Ok(true) => {
                            // Continue draining compaction debt in this wake cycle.
                        }
                        Ok(false) => {
                        ...
                        Err(e) => {
                            ...
                            core.error_handler()
                                .set_error(e, BackgroundErrorReason::Compaction);
                            break;
                        }
                    }
                }
    ...
    pub(crate) fn wake_up_memtable(&self) {
        // Always notify.
        // If the worker is currently running, Notify stores a permit so the next
        // loop iteration runs immediately after the current one. Gating on
        // `memtable_running` can drop wake-ups and leave immutable memtables
        // stranded until another unrelated write arrives.
        self.memtable_notify.notify_one();
    }

    #[cfg(test)]
    pub(crate) fn wake_up_level(&self) {
        // Same rationale as wake_up_memtable: avoid dropping wake-ups when the
        // level worker is busy.
...
mod tests {
    ...
    impl CompactionOperations for MockCoreInner {
        ...
        fn compact(&self, _strategy: Arc<dyn CompactionStrategy>) -> Result<bool> {
            let start = std::time::Instant::now();
    ...
    async fn test_already_running_tasks() {
        let opts = Arc::new(Options::default());
        // Create core with longer delays so wake-ups can race with active workers.
        let core = Arc::new(MockCoreInner::with_delays(100, 100));
        let task_manager =
            TaskManager::new(Arc::clone(&core) as Arc<dyn CompactionOperations>, opts);

        // Wake memtable twice back-to-back.
        task_manager.wake_up_memtable();
        task_manager.wake_up_memtable();

        // Notify may coalesce back-to-back signals into a single permit.
        // The key invariant here is that the worker runs and does not stall.
        time::sleep(Duration::from_millis(280)).await;
        assert!(
            core.memtable_compactions.load(Ordering::SeqCst) >= 1,
            "Memtable worker should run after wake-up signals"
        );
        ...
        // Level worker should also run; exact count is not deterministic because
        // Notify may coalesce permits.
        assert!(
            core.level_compactions.load(Ordering::SeqCst) >= 2,
            "Level worker should run for both memtable-triggered and direct wake-ups"
        );
