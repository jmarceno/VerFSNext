...
impl Comparator for InternalKeyComparator {
    ...
    fn separator(&self, a: &[u8], b: &[u8]) -> Vec<u8> {
        ...
        if self
            .user_comparator
            .compare(key_a.user_key.as_ref(), key_b.user_key.as_ref())
            != Ordering::Equal
        {
            let sep = self
                .user_comparator
    ...
    fn successor(&self, key: &[u8]) -> Vec<u8> {
        ...
        let user_key_succ = self
            .user_comparator
            .successor(internal_key.user_key.as_ref());
        ...
        if user_key_succ.len() <= internal_key.user_key.len()
            && self
                .user_comparator
                .compare(internal_key.user_key.as_ref(), &user_key_succ)
                == Ordering::Less
        {
            let result = InternalKey::new(
                user_key_succ,
...
mod tests {
    ...
    fn test_separator_successor_randomized() {
        ...
        for _ in 0..1000 {
            ...
            assert!(
                succ >= s1,
                "Successor must be >= original. s1={:?}, succ={:?}",
                s1,
                succ
            );
        }
